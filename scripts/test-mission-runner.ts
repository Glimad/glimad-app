...
import {
  ...
  executeMission,
  resumeMissionAfterInput,
} from "../lib/missions/runner";
...
async function testHappyPath(projectId: string) {
  ...
  ok(
    ...
    inst1?.status === "waiting_input",
    `got: ${inst1?.status}`,
  );

  const { data: steps1 } = await sb()
    .from("mission_steps")
    .select("step_number, status")
    .eq("mission_instance_id", instanceId)
    .order("step_number");
  const completed1 = (steps1 ?? []).filter(
    (s: { status: string }) => s.status === "completed",
  );
  ok(
    "steps 1+2 completed before pause",
    completed1.length >= 2,
    `completed: ${completed1.length}`,
  );
...
async function testBrainUpdate(projectId: string) {
  ...
  const factMap = Object.fromEntries(
    (facts ?? []).map((f: { fact_key: string; value: unknown }) => [
      f.fact_key,
      f.value,
    ]),
  );
  ok(
    "audience_persona fact saved (user input)",
    factMap["audience_persona"] === "Millennials viajeros",
    `got: ${JSON.stringify(factMap["audience_persona"])}`,
  );
  ok(
    "positioning fact saved (user input)",
    factMap["positioning"] === "Fotografías que cuentan historias",
...
async function testLedgerDebit(projectId: string) {
  ...
  ok(
    "amount_allowance is positive",
    (ledger?.[0]?.amount_allowance ?? 0) > 0,
    `got: ${ledger?.[0]?.amount_allowance}`,
  );
  ok(
    "amount_allowance = 5",
    ledger?.[0]?.amount_allowance === 5,
    `got: ${ledger?.[0]?.amount_allowance}`,
  );
}
...
async function testWalletDecrement(projectId: string) {
  ...
  const { data: wallet } = await sb()
    ...
    .single();
  ok(
    "wallet balance = 2000 - 5 = 1995",
    wallet?.allowance_llm_balance === 1995,
...
async function testWriteOutputs(projectId: string) {
  ...
  ok(
    "mission_instance_id linked",
    outputs?.[0]?.mission_instance_id === instanceId,
  );

  // Draft calendar items should also be created
  const outputIds = (outputs ?? []).map((o: { id: string }) => o.id);
  const { data: calItems } = await sb()
    .from("core_calendar_items")
    .select("id, state, output_id")
    .eq("project_id", projectId)
    .in("output_id", outputIds);
  ok(
    "draft calendar items created",
    (calItems?.length ?? 0) > 0,
    `count: ${calItems?.length}`,
  );
  ok(
    "calendar items are draft",
    calItems?.every((c: { state: string }) => c.state === "draft"),
  );
}
...
async function testOutputsOnInstance(projectId: string) {
  ...
  ok(
    ...
    inst?.outputs !== null && inst?.outputs !== undefined,
...
async function testMissionCompletedSignal(projectId: string) {
  ...
  ok(
    "signal has template_code",
    sigVal?.["template_code"] === "NICHE_CONFIRM_V1",
...
async function testStepLogs(projectId: string) {
  ...
  ok(
    "all steps completed",
    steps?.every((s: { status: string }) => s.status === "completed"),
