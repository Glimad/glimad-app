...
async function main() {
  ...
  await admin.from("core_wallets").upsert(
    {
      ...
      premium_credits_balance: 1250,
      premium_daily_cap_remaining: 1250,
    ...
    [
      {
        project_id: projectId,
        fact_key: "niche_raw",
        value: "personal finance for remote workers",
        source: "onboarding",
        updated_at: new Date().toISOString(),
      },
      {
        project_id: projectId,
        fact_key: "focus_platform",
        value: "instagram",
        source: "onboarding",
        updated_at: new Date().toISOString(),
      },
      ...
      },
      {
        ...
        value: "grow to 5K followers in 3 months",
        source: "onboarding",
        updated_at: new Date().toISOString(),
      },
    ],
  ...
  console.log(
    "[1] AUDIENCE_PERSONA_V1 — generates persona, confirms, completes",
  );
  {
    ...
    const { data: inst } = await admin
      ...
      .single();
    ok(
      "status = waiting_input after first run",
    ...
    ok(
      "brain_read completed",
      steps?.find((s) => s.step_type === "brain_read")?.status === "completed",
    );
    ok(
      "llm_text completed",
      steps?.find((s) => s.step_type === "llm_text")?.status === "completed",
    );
    ...
    ok(
      "LLM output has persona_name",
      !!llmOut?.persona_name,
      `keys: ${Object.keys(llmOut ?? {}).join(", ")}`,
    );
    ...
    await resumeMissionAfterInput(admin, instanceId, {
      persona_name: String(llmOut?.persona_name ?? "Test Persona"),
    });
  ...
  );
  {
    ...
    const llmOut = llmStep?.output as Record<string, unknown>;
    ok(
      "LLM output has posts_per_week",
      llmOut?.posts_per_week !== undefined,
      `keys: ${Object.keys(llmOut ?? {}).join(", ")}`,
    );
    ok(
      "LLM output has best_posting_times",
      Array.isArray(llmOut?.best_posting_times),
    );

    await resumeMissionAfterInput(admin, instanceId, {
      posts_per_week: String(llmOut?.posts_per_week ?? "3"),
    });
    const { data: completed } = await admin
      .from("mission_instances")
      .select("status")
  ...
  );
  {
    ...
    const llmOut = llmStep?.output as Record<string, unknown>;
    ok(
      "LLM output has tone_of_voice",
      !!llmOut?.tone_of_voice,
      `keys: ${Object.keys(llmOut ?? {}).join(", ")}`,
    );
    ok(
      "LLM output has content_pillars",
      Array.isArray(llmOut?.content_pillars),
    );
