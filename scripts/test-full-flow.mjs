...
function warn(label, detail='') { console.log(`  ⚠️  ${label}${detail ? ': '+detail : ''}`); warns++ }
...
async function getProject(userId) {
  const { data } = await admin.from('projects').select('*').eq('user_id', userId).neq('status', 'archived').single()
  return data
}
...
// ── USERS ─────────────────────────────────────────────────────────────────────
const USERS = [
  { email: 'alice@test.glimad.com', label: 'Alice (F3/monetize/PRO)',  expectedPhase: 'F3' },
  { email: 'bob@test.glimad.com',   label: 'Bob (F0/test/BASE)',       expectedPhase: 'F0' },
  { email: 'carol@test.glimad.com', label: 'Carol (F5/scale/ELITE)',   expectedPhase: 'F5' },
]
...
async function testStep2_Database() {
  ...
  const tables = [
    'projects', 'user_preferences', 'brain_facts', 'brain_facts_history',
    'brain_signals', 'brain_snapshots', 'core_phase_runs', 'core_policy_runs',
    'mission_templates', 'mission_instances', 'mission_steps',
    'core_outputs', 'core_calendar_items', 'core_plans', 'core_subscriptions',
    'core_wallets', 'core_ledger', 'stripe_events', 'core_payments',
    'platform_metrics', 'core_scrape_runs', 'core_jobs',
    'monetization_products', 'monetization_events', 'notifications',
    'service_requests_backlog', 'onboarding_sessions', 'event_log',
    'pulse_runs', 'feature_flags',
  ]
  ...
  const { data: miRow } = await admin.from('mission_instances').select('executor_type, handoff_channel, service_case_id').limit(1)
  if (miRow !== null) ok('mission_instances has executor_type/handoff_channel/service_case_id')
  else fail('mission_instances missing services columns')

  // brain_facts uses `value` column (not value_json)
  const { data: bfRow } = await admin.from('brain_facts').select('fact_key, value').limit(1)
...
async function testStep3_Auth(token, user, project) {
  ...
  else { fail('Project missing'); return }

  const { status, data } = await api(token, 'GET', '/api/me/access')
  if (status === 200 && data.access_state === 'active') ok('/api/me/access → active')
...
async function testStep4_Subscription(project) {
  ...
  const { data: sub } = await admin.from('core_subscriptions').select('*').eq('project_id', project.id).single()
  if (sub && sub.status === 'active') ok(`core_subscriptions active (${sub.plan_code})`)
  else fail('core_subscriptions missing or not active')

  const { data: wallet } = await admin.from('core_wallets').select('*').eq('project_id', project.id).single()
  if (wallet) ok(`core_wallets: allowance=${wallet.allowance_llm_balance}, premium=${wallet.premium_credits_balance}`)
  else fail('core_wallets missing')

  const { data: ledger } = await admin.from('core_ledger').select('ledger_id').eq('project_id', project.id).limit(5)
...
async function testStep5_BrainSeed(project) {
  ...
  const { data: facts } = await admin.from('brain_facts').select('fact_key, value').eq('project_id', project.id)
  const factMap = Object.fromEntries((facts ?? []).map(f => [f.fact_key, f.value]))

  const requiredFacts = ['platforms.focus', 'identity.niche', 'identity.primary_goal', 'capabilities.on_camera_comfort']
  ...
  const CORE_FLOW = ['VISION_PURPOSE_MOODBOARD_V1','CONTENT_COMFORT_STYLE_V1','PLATFORM_STRATEGY_PICKER_V1','NICHE_CONFIRM_V1','PREFERENCES_CAPTURE_V1']
  const { data: missions } = await admin.from('mission_instances').select('template_code, status').eq('project_id', project.id)
...
async function testStep6_Brain(token, project) {
  ...
  const { data: hist } = await admin.from('brain_facts_history').select('*').eq('project_id', project.id).limit(5)
  if (hist?.length > 0) ok(`brain_facts_history: ${hist.length} entries (DB trigger working)`)
  else warn('brain_facts_history empty')

  // Brain history API
  const { status } = await api(token, 'GET', '/api/brain/history?project_id=' + project.id + '&fact_key=identity.niche')
  if (status === 200 || status === 404) ok('/api/brain/history endpoint responds')
  else fail('/api/brain/history error', String(status))

  const { data: signals } = await admin.from('brain_signals').select('*').eq('project_id', project.id).limit(5)
  if (signals?.length > 0) ok(`brain_signals: ${signals.length} entries`)
  else warn('brain_signals empty for this project')

  const { data: snaps } = await admin.from('brain_snapshots').select('*').eq('project_id', project.id).limit(3)
...
async function testStep7_Scrape(token, project) {
  ...
  const { data: metrics } = await admin.from('platform_metrics').select('*').eq('project_id', project.id).limit(3)
  ...
  const { status } = await api(token, 'POST', '/api/scrape/request', { platform: 'instagram', handle: 'testhandle' })
  if (status === 200 || status === 409 || status === 200) ok(`/api/scrape/request responds (${status})`)
...
async function testStep8_PhaseEngine(token, project, user) {
  ...
  const { status, data } = await api(token, 'POST', '/api/engines', { force: true })
  if (status === 200 && data.phaseResult?.phase) {
    ...
    if (d && typeof d.execution === 'number' && typeof d.audienceSignal === 'number' && typeof d.clarity === 'number' && typeof d.readiness === 'number') {
      ok(`4D scores: exec=${d.execution} audience=${d.audienceSignal} clarity=${d.clarity} ready=${d.readiness}`)
    } else fail('Missing 4D dimension scores')
    if (pr.gates) ok(`Gates map present (${Object.keys(pr.gates).length} keys)`)
  } else fail('Phase Engine failed', JSON.stringify(data).slice(0,200))

  const { data: proj } = await admin.from('projects').select('phase_code').eq('id', project.id).single()
...
async function testStep9_InflexionEngine(token) {
  ...
  if (status === 200 && data.inflexion !== undefined) {
    ...
    ok(`Inflexion Engine: type=${inf?.type ?? 'none'}, confidence=${inf?.confidence ?? 0}`)
  } else fail('Inflexion Engine missing from response', JSON.stringify(data).slice(0,100))
}
...
async function testStep10_PolicyEngine(token, user) {
  ...
  if (status === 200 && data.policy?.activeMode) {
    ...
    if (['test','scale','monetize'].includes(p.activeMode)) ok(`Active mode is valid: ${p.activeMode}`)
    else fail('Invalid activeMode', p.activeMode)
  } else fail('Policy Engine missing from response', JSON.stringify(data).slice(0,100))
}

async function testStep11_MissionRunner(token, project) {
  section(`STEP 11 — Mission Runner`)

  // Find any mission (queued, running, needs_user_input, or completed)
  const { data: anyMission } = await admin.from('mission_instances')
  ...
  if (!anyMission) { warn('No missions at all for this project'); return }
  ok(`Mission found: ${anyMission.template_code} (${anyMission.status})`)

  // GET mission detail
  const { status, data } = await api(token, 'GET', `/api/missions/${anyMission.id}`)
  if (status === 200 && data.instance) {
    ok(`GET /api/missions/[id] → ${data.instance.status}`)
    if (data.steps !== undefined) ok(`Steps field present (${data.steps.length})`)
  } else fail('GET /api/missions/[id] failed', JSON.stringify(data).slice(0,100))

  // /api/missions/start — try starting a new batch mission
  const { status: ss, data: sd } = await api(token, 'POST', '/api/missions/start', { template_code: 'CONTENT_BATCH_3D_V1' })
  if (ss === 200 && sd.instance_id) ok(`/api/missions/start → instance ${sd.instance_id.slice(0,8)}`)
  else if (ss === 402) warn('/api/missions/start → insufficient credits')
  else warn('/api/missions/start responded with ' + ss, JSON.stringify(sd).slice(0,100))
}
...
async function testStep12_Studio(token, project) {
  ...
  const { status: ts, data: td } = await api(token, 'GET', '/api/studio/topics')
  if (ts === 200 && td.platform) ok(`/api/studio/topics GET: platform=${td.platform}, limit=${td.caption_limit}`)
  else fail('/api/studio/topics GET failed', JSON.stringify(td))

  // POST /api/studio/topics → generates topic ideas
  const { status: pts, data: ptd } = await api(token, 'POST', '/api/studio/topics', { content_type: 'reel' })
  if (pts === 200 && Array.isArray(ptd.topics)) ok(`/api/studio/topics POST: ${ptd.topics.length} topics`)
  else if (pts === 429) warn('/api/studio/topics POST → rate limited')
  else fail('/api/studio/topics POST failed', `status=${pts}`)

  // POST /api/studio/generate
  const { status: gs, data: gd } = await api(token, 'POST', '/api/studio/generate', {
    content_type: 'reel',
    topic: 'quick productivity tip',
  })
  if (gs === 200 && gd.content) {
    ok(`/api/studio/generate → content generated (hook: ${String(gd.content.hook ?? '').slice(0,50)}...)`)
  ...
  const { data: outputs } = await admin.from('core_outputs').select('id').eq('project_id', project.id).limit(3)
...
async function testStep13_Calendar(token, project) {
  ...
  if (status === 200) {
    ok(`/api/calendar: ${data.items?.length ?? 0} scheduled, ${data.drafts?.length ?? 0} drafts`)
    // Verify `status` field (not `state`)
    const allItems = [...(data.items ?? []), ...(data.drafts ?? [])]
    if (allItems.length > 0) {
      const item = allItems[0]
      if ('status' in item && !('state' in item)) ok('Calendar items use status column correctly')
      else if ('state' in item) fail('Calendar items have state instead of status — column bug')
    }
...
async function testStep14_Pulse(token, project) {
  ...
  const { data: pulses } = await admin.from('pulse_runs').select('*').eq('project_id', project.id).limit(2)
...
async function testStep15_Wallet(token, project) {
  ...
  const { data: wallet } = await admin.from('core_wallets').select('*').eq('project_id', project.id).single()
  if (!wallet) { fail('Wallet missing'); return }
  ok(`Wallet: allowance=${wallet.allowance_llm_balance}, premium=${wallet.premium_credits_balance}, status=${wallet.status}`)

  const { data: ledger } = await admin.from('core_ledger').select('ledger_id, kind').eq('project_id', project.id).limit(10)
...
async function testStep16_Gamification(project) {
  ...
  const { data: proj } = await admin.from('projects').select('xp, energy, streak_days').eq('id', project.id).single()
  if (!proj) { fail('Project not found'); return }
  ...
  if (typeof proj.streak_days === 'number') ok(`Streak: ${proj.streak_days} days`)
...
async function testStep17_Dashboard(token, project, user) {
  ...
  const { status, data } = await api(token, 'POST', '/api/engines', {})
  if (status === 200 && data.phaseResult) ok('All 3 engines (phase/inflexion/policy) respond for dashboard')
  else fail('Engines call for dashboard failed')

  // Monetization KPIs — returns {kpis: {...}}
  const { status: ks, data: kd } = await api(token, 'GET', '/api/monetization/kpis')
  if (ks === 200 && kd.kpis != null) ok(`Monetization KPIs: revenue=${kd.kpis.totalRevenue ?? 0}`)
  else warn('Monetization KPIs unexpected', JSON.stringify(kd).slice(0,100))

  // Quick stats facts
  const { data: qfacts } = await admin.from('brain_facts').select('fact_key').eq('project_id', project.id)
    .in('fact_key', ['followers_total','avg_engagement_rate','current_followers'])
  ok(`Quick stats facts in brain: ${qfacts?.map(f=>f.fact_key).join(', ') || 'none'}`)
}
...
async function testStep18_Monetization(token, project, user) {
  ...
  if (ks === 200 && kd.kpis != null) {
    ok(`/api/monetization/kpis: rev=€${kd.kpis.totalRevenue ?? 0} mrr=€${kd.kpis.mrr ?? 0} streams=${kd.kpis.activeStreams ?? 0}`)
  } else fail('/api/monetization/kpis failed', JSON.stringify(kd))

  // Products
  const { status: ps, data: pd } = await api(token, 'GET', '/api/monetization/products')
  if (ps === 200) {
  ...
  if (phaseRank >= 3) {
    const { status: ss, data: sd } = await api(token, 'POST', '/api/monetization/suggest')
    if (ss === 200 && sd.suggestion?.name) ok(`/api/monetization/suggest: "${sd.suggestion.name}" (${sd.suggestion.product_type})`)
    else fail('/api/monetization/suggest failed', `status=${ss} ${JSON.stringify(sd).slice(0,100)}`)
...
async function testStep19_Notifications(token, project) {
  ...
  if (status === 200) ok(`/api/notifications: ${data.notifications?.length ?? 0} unread`)
  else fail('/api/notifications failed', String(status))

  const { data: all } = await admin.from('notifications').select('id, type, read_at').eq('project_id', project.id).limit(5)
  if (all?.length > 0) ok(`DB notifications: ${all.length} total for this project`)
  ...
  if (unread.length > 0) {
    const { status: ps } = await api(token, 'PATCH', `/api/notifications/${unread[0].id}`)
...
async function testStep20_Services(project) {
  ...
  const { data: mi } = await admin.from('mission_instances')
  ...
  const { data: facts } = await admin.from('brain_facts')
    .select('fact_key').eq('project_id', project.id)
    .in('fact_key', ['services_preference_channel', 'services_preference_mode_default'])
  if (facts?.length > 0) ok(`Services preference facts stored: ${facts.map(f=>f.fact_key).join(', ')}`)
  else warn('Services preference facts not written (requires PREFERENCES_CAPTURE_V1 completion)')

  const { error } = await admin.from('service_requests_backlog').select('*').limit(1)
...
async function testStep21_Admin(token, project) {
  ...
  const { data: proj } = await admin.from('projects').select('is_admin').eq('id', project.id).single()
...
async function main() {
  ...
  for (const user of USERS) {
    ...
    try {
      ...
      if (!project) { fail(`Project not found for ${user.email}`); continue }
      ok(`Authenticated as ${user.email} → project ${project.id.slice(0,8)} (phase=${project.phase_code})`)
    } catch (e) {
