...
async function countInflexionSignals(projectId: string) {
  ...
  const { data } = await admin
    ...
    .select("id")
    .eq("project_id", projectId)
    .eq("signal_key", "inflexion_detected");
...
async function testViralSpike(projectId: string, token: string) {
  ...
  ok(
    "inflexion type = viral_spike",
    result?.inflexion?.type === "viral_spike",
    `got ${result?.inflexion?.type}`,
  );
  ok(
    "confidence >= 0.5",
    (result?.inflexion?.confidence ?? 0) >= 0.5,
    `got ${result?.inflexion?.confidence}`,
  );

  const event = await getLatestInflexionEvent(projectId);
  ok("core_inflexion_events row written", !!event, "row missing");
  ok(
    "event_key = viral_spike",
    event?.event_key === "viral_spike",
    `got ${event?.event_key}`,
  );
  ok("severity = high", event?.severity === "high", `got ${event?.severity}`);
  ok(
    "recommended_actions contains VIRAL_RESPONSE_V1",
    event?.recommended_actions?.includes("VIRAL_RESPONSE_V1"),
    JSON.stringify(event?.recommended_actions),
  );

  const sigCount = await countInflexionSignals(projectId);
  ok("inflexion_detected signal written", sigCount >= 1, `count=${sigCount}`);
  ok(
    "policy topMission influenced by inflexion",
    !!result?.policy?.topMission,
    `topMission=${result?.policy?.topMission}`,
  );
  ...
  ok(
    "reach-based viral_spike detected",
    result2?.inflexion?.type === "viral_spike",
    `got ${result2?.inflexion?.type}`,
  );
  ok(
    "confidence >= 0.8",
    (result2?.inflexion?.confidence ?? 0) >= 0.8,
    `got ${result2?.inflexion?.confidence}`,
  );
  ...
  await seedSignal(
    ...
    { value: 1300 },
    1 * 24,
  ); // 24h ago (in 72h window)
  await seedSignal(
    projectId,
    "growth.followers_total",
    { value: 1000 },
    3 * 24,
  ); // 72h ago (edge of 72h window, also 30d)
  await seedSignal(
    projectId,
    "growth.followers_total",
    { value: 1000 },
    10 * 24,
  ); // 10d ago (30d window, for baseline)
  await seedSignal(
    projectId,
    "growth.followers_total",
    { value: 700 },
    30 * 24,
  ); // 30d ago (oldest, baseline start)
  ...
  ok(
    "growth-based viral_spike detected",
    result3?.inflexion?.type === "viral_spike",
    `got ${result3?.inflexion?.type}`,
  );
  ok(
    "confidence >= 0.7",
    (result3?.inflexion?.confidence ?? 0) >= 0.7,
    `got ${result3?.inflexion?.confidence}`,
  );
  ok(
    "evidence has follower_surge",
    !!result3?.inflexion?.evidence?.follower_surge,
    `evidence: ${JSON.stringify(result3?.inflexion?.evidence)}`,
  );
}
...
async function testConsistencyBreak(projectId: string, token: string) {
  ...
  ok("engines returns 200", !!result, "null response");
  ok(
    "inflexion type = consistency_break",
    result?.inflexion?.type === "consistency_break",
    `got ${result?.inflexion?.type}`,
  );
  ok(
    "confidence = 0.80",
    result?.inflexion?.confidence === 0.8,
    `got ${result?.inflexion?.confidence}`,
  );
  ok(
    "evidence.no_recent_post = true",
    result?.inflexion?.evidence?.no_recent_post === true,
    `got ${result?.inflexion?.evidence?.no_recent_post}`,
  );

  const event = await getLatestInflexionEvent(projectId);
  ok("core_inflexion_events row written", !!event, "row missing");
  ok(
    "event_key = consistency_break",
    event?.event_key === "consistency_break",
    `got ${event?.event_key}`,
  );
  ok("type = alert", event?.type === "alert", `got ${event?.type}`);
  ok("severity = high", event?.severity === "high", `got ${event?.severity}`);
  ok(
    "recommended_actions contains RESCUE_CONSISTENCY_V1",
    event?.recommended_actions?.includes("RESCUE_CONSISTENCY_V1"),
    JSON.stringify(event?.recommended_actions),
  );
  ...
  ok("engines returns 200", !!result2, "null response");
  ok(
    "consistency_break suppressed when recent post exists",
    result2?.inflexion?.type !== "consistency_break",
    `got: ${result2?.inflexion?.type}`,
  );
}
...
async function testMonetizeReady(projectId: string, token: string) {
  ...
  ok("engines returns 200", !!result, "null response");
  ok(
    "inflexion type = monetize_ready",
    result?.inflexion?.type === "monetize_ready",
    `got ${result?.inflexion?.type}`,
  );
  ok(
    "confidence = 0.8",
  ...
  ok(
    "recommended_actions contains DEFINE_OFFER_V1",
    event?.recommended_actions?.includes("DEFINE_OFFER_V1"),
    JSON.stringify(event?.recommended_actions),
  );
  ...
  ok("engines returns 200", !!result2, "null response");
  ok(
    "monetize_ready suppressed within 90d window",
    result2?.inflexion?.type !== "monetize_ready",
    `got: ${result2?.inflexion?.type}`,
  );
}
...
async function testPlateau(projectId: string, token: string) {
  ...
  ok("engines returns 200", !!result, "null response");
  ok(
    "inflexion type = plateau",
    result?.inflexion?.type === "plateau",
    `got ${result?.inflexion?.type}`,
  );
  ok(
    "confidence >= 0.7",
    (result?.inflexion?.confidence ?? 0) >= 0.7,
    `got ${result?.inflexion?.confidence}`,
  );
  ...
  ok("engines returns 200", !!result2, "null response");
  ok(
    "plateau suppressed when delta > 0",
    result2?.inflexion?.type !== "plateau",
    `got: ${result2?.inflexion?.type}`,
  );
  ...
  ok("engines returns 200", !!result3, "null response");
  ok(
    "plateau suppressed when any ER ≥ 2%",
    result3?.inflexion?.type !== "plateau",
    `got: ${result3?.inflexion?.type}`,
  );
}
...
async function testDowngradeCandidate(projectId: string, token: string) {
  ...
  ok("engines returns 200", !!result, "null response");
  ok(
    "inflexion type = downgrade_candidate",
    result?.inflexion?.type === "downgrade_candidate",
    `got ${result?.inflexion?.type}`,
  );
  ok(
    "confidence >= 0.6",
    (result?.inflexion?.confidence ?? 0) >= 0.6,
    `got ${result?.inflexion?.confidence}`,
  );
  ok(
    "evidence has recent_posts_30d",
    result?.inflexion?.evidence?.recent_posts_30d === 8,
  ...
  ok("core_inflexion_events row written", !!event, "row missing");
  ok(
    "event_key = downgrade_candidate",
    event?.event_key === "downgrade_candidate",
    `got ${event?.event_key}`,
  );
  ...
  ok("engines returns 200", !!result2, "null response");
  ok(
    "downgrade_candidate suppressed when gap < 5 days",
    result2?.inflexion?.type !== "downgrade_candidate",
    `got: ${result2?.inflexion?.type}`,
  );
  ...
  ok("engines returns 200", !!result3, "null response");
  ok(
    "downgrade_candidate suppressed when frequency stable/increasing",
    result3?.inflexion?.type !== "downgrade_candidate",
    `got: ${result3?.inflexion?.type}`,
  );
}
...
async function testNoInflexion(projectId: string, token: string) {
  ...
  ok("engines returns 200", !!result, "null response");
  ok(
    "inflexion = null (no event)",
    result?.inflexion === null,
    `got type=${result?.inflexion?.type}`,
  );

  const sigCount = await countInflexionSignals(projectId);
  ok(
    "no inflexion_detected signal written",
    sigCount === 0,
    `count=${sigCount}`,
  );
}
...
async function testCooldown(projectId: string, token: string) {
  ...
  ok(
    "first call inflexion detected",
    result1?.inflexion?.type === "viral_spike",
    `got ${result1?.inflexion?.type}`,
  );
  ...
  const result2 = await runEngines(token);
  ok(
    "second call still returns inflexion result (passed through)",
    result2?.inflexion?.type === "viral_spike",
    `got ${result2?.inflexion?.type}`,
  );
  ...
  ok(
    "no duplicate signal on second call",
    sigCount2 === 1,
    `count=${sigCount2}`,
  );
  ...
  const { data: events } = await admin
    ...
    .select("id")
    .eq("project_id", projectId)
    .eq("event_key", "viral_spike");
  ok(
    "only 1 core_inflexion_events row (no duplicate)",
    (events?.length ?? 0) === 1,
    `count=${events?.length}`,
  );
}
