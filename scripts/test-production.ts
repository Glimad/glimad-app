...
async function ensureProject(userId: string): Promise<string> {
  ...
  if (existing) {
    ...
    const { data: wallet } = await admin.from('core_wallets')
      .select('wallet_id').eq('project_id', existing.id).single()
    if (!wallet) {
...
async function testScrapeRunAuthorized(projectId: string) {
  ...
  const { data: job } = await admin.from('core_jobs').insert({
    ...
    user_id: (await admin.from('projects').select('user_id').eq('id', projectId).single()).data!.user_id,
  ...
  if (jobResult) {
    ok('test job result is done or failed', ['done', 'failed', 'rate_limited'].some(s => jobResult.result?.includes(s) || jobResult.result === 'done'), jobResult.result)
  }

  // Check job status in DB
  const { data: updatedJob } = await admin.from('core_jobs')
    .select('status').eq('job_id', job.job_id).single()
  ok('job no longer queued in DB', updatedJob?.status !== 'queued', `status=${updatedJob?.status}`)
...
async function testEnginesEndpoint(token: string, projectId: string) {
  ...
  const body = await res.json()
  ok('phaseResult present', !!body.phaseResult, JSON.stringify(body).slice(0, 200))
  ok('phase is valid F-code', /^F[0-7]$/.test(body.phaseResult?.phase ?? ''), `got ${body.phaseResult?.phase}`)
  ok('capabilityScore 0-100', body.phaseResult?.capabilityScore >= 0 && body.phaseResult?.capabilityScore <= 100,
    `got ${body.phaseResult?.capabilityScore}`)
  ok('dimensionScores present', !!body.phaseResult?.dimensionScores)
  ok('8 dimension scores', Object.keys(body.phaseResult?.dimensionScores ?? {}).length === 8,
    `got ${Object.keys(body.phaseResult?.dimensionScores ?? {}).join(', ')}`)
  ok('confidence 0-1', body.phaseResult?.confidence >= 0 && body.phaseResult?.confidence <= 1,
  ...
  console.log(`  Dimensions: ${JSON.stringify(body.phaseResult?.dimensionScores)}`)
...
async function testDBStateAfterEngines(projectId: string, phaseResult: any) {
  ...
  // core_phase_runs
  const { data: run } = await admin.from('core_phase_runs')
    .select('*').eq('project_id', projectId)
    .order('computed_at', { ascending: false }).limit(1).single()
  ok('core_phase_runs row written', !!run)
  ok('phase_code matches', run?.phase_code === phaseResult?.phase, `DB=${run?.phase_code}, result=${phaseResult?.phase}`)
  ok('capability_score matches', run?.capability_score === phaseResult?.capabilityScore)
  ...
  ok('current_phase fact written', factMap['current_phase'] === phaseResult?.phase,
    `got ${factMap['current_phase']}`)
  ok('phase_scores fact written', !!factMap['phase_scores'])
  ok('capability_score fact written', factMap['capability_score'] === phaseResult?.capabilityScore,
    `got ${factMap['capability_score']}`)

  // projects table
  const { data: project } = await admin.from('projects')
    .select('phase_code').eq('id', projectId).single()
  ok('projects.phase_code updated', project?.phase_code === phaseResult?.phase,
...
async function testScrapeRequest(token: string, projectId: string) {
  ...
  ok('status is queued or rate_limited', ['queued', 'rate_limited', 'running', 'done'].includes(body.status),
    `got ${body.status}`)
