...
const SERVICE_ROLE_KEY =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImF3YWFrdXJ2bm5nYXptbm5td3phIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NDg1NDA0MSwiZXhwIjoyMDgwNDMwMDQxfQ.placeholder";
...
function daysDate(n) {
  return days(n).slice(0, 10);
}
function ago(n) {
  return days(-n);
}
function agoDate(n) {
  return ago(n).slice(0, 10);
}
...
async function ins(table, rows, { onConflict } = {}) {
  ...
  if (error) {
    ...
    throw new Error(
      `insert ${table}: ${error.message} | ${JSON.stringify(arr[0]).slice(0, 200)}`,
    );
  }
  return data ?? [];
}
...
async function upsert(table, rows, conflict) {
  ...
  const { data, error } = await admin
    .from(table)
    .upsert(arr, { onConflict: conflict })
    .select();
...
function buildMissionInstance(
  ...
  return {
    ...
    current_step:
      status === "completed" ? 5 : status === "waiting_input" ? 3 : 0,
...
async function main() {
  ...
  const alice = await upsertUser("alice@test.glimad.com", "Seedpass123!", {
    locale: "en",
  });
  const bob = await upsertUser("bob@test.glimad.com", "Seedpass123!", {
    locale: "en",
  });
  const carol = await upsertUser("carol@test.glimad.com", "Seedpass123!", {
    locale: "es",
  });
  ...
  await admin
    ...
    .delete()
    .eq("converted_to_user_id", alice.id);
  await admin
    .from("onboarding_sessions")
    .delete()
    .eq("converted_to_user_id", bob.id);
  await admin
    .from("onboarding_sessions")
    .delete()
    .eq("converted_to_user_id", carol.id);
    ...
    [
      {
        ...
        allowance_reset_at: days(25),
        premium_reset_at: days(25),
        status: "active",
      },
      {
        project_id: bobProjId,
        ...
        premium_daily_cap_remaining: 50,
        allowance_reset_at: days(28),
        premium_reset_at: days(28),
        status: "active",
      },
      {
        project_id: carolProjId,
        ...
        premium_daily_cap_remaining: 250,
        allowance_reset_at: days(20),
        premium_reset_at: days(20),
        status: "active",
      },
    ],
    "project_id",
  ...
  await ins("core_ledger", [
    ...
    },
    {
      project_id: aliceProjId,
      kind: "debit",
      amount_allowance: -800,
      amount_premium: 0,
      reason_key: "MISSION_ALLOWANCE_DEBIT",
      idempotency_key: `seed_mission_debit_alice_1`,
    ...
    },
    {
      ...
      idempotency_key: `seed_scrape_debit_alice_1`,
    ...
    },
    {
      ...
      amount_premium: -5,
      reason_key: "SCRAPE_LIGHT_DEBIT",
      idempotency_key: `seed_scrape_debit_alice_2`,
      metadata_json: { platform: "instagram" },
    },
    {
      project_id: aliceProjId,
      ...
      amount_allowance: 0,
      amount_premium: 100,
      reason_key: "MISSION_COMPLETION_REWARD",
    ...
    },
    {
      project_id: carolProjId,
      kind: "debit",
      amount_allowance: -1500,
      amount_premium: 0,
      reason_key: "MISSION_ALLOWANCE_DEBIT",
      idempotency_key: `seed_mission_debit_carol_1`,
      metadata_json: {},
    },
    {
      project_id: carolProjId,
      ...
      amount_premium: -200,
      reason_key: "SCRAPE_LIGHT_DEBIT",
      idempotency_key: `seed_scrape_debit_carol_1`,
      metadata_json: { platform: "youtube" },
    },
    {
      project_id: carolProjId,
      kind: "debit",
      ...
      amount_premium: -200,
      reason_key: "SCRAPE_LIGHT_DEBIT",
      idempotency_key: `seed_scrape_debit_carol_2`,
      metadata_json: { platform: "instagram" },
    },
    {
      project_id: carolProjId,
      ...
      amount_premium: 100,
      reason_key: "MISSION_COMPLETION_REWARD",
      idempotency_key: `seed_reward_carol_streak14`,
      metadata_json: { streak_milestone: 14 },
    },
    {
      project_id: carolProjId,
      ...
      amount_premium: 0,
      reason_key: "PLAN_MONTHLY_GRANT",
      idempotency_key: `seed_reward_carol_streak30`,
      metadata_json: { streak_milestone: 30 },
    },
    {
      project_id: carolProjId,
      kind: "debit",
      amount_allowance: -100,
      amount_premium: 0,
      reason_key: "LLM_CALL_STUDIO",
      idempotency_key: `seed_studio_carol_1`,
      metadata_json: {},
    },
  ]);
  ...
  await upsert(
    "brain_facts",
    buildFacts(aliceProjId, {
      "identity.niche": {
        niche: "Personal finance",
        subniche: "Investing for millennials",
        target_audience: "Women 25-35 building wealth",
        unique_angle: "Fun and jargon-free financial education",
      },
      "identity.primary_goal":
        "Reach 20k followers and launch first digital product",
      "identity.main_blocker": "Not enough time to create consistent content",
      "identity.flow_variant": "B_has_presence",
      "identity.north_star":
        "Democratize financial education for young women in Spain",
      ...
      "platforms.satellites": [
        { platform: "tiktok", handle: "@alicefinance", status: "planned" },
      ],
      ...
      current_phase: "F3",
      vision_statement:
        "Help 100,000 women take control of their finances through engaging content",
      ...
      audience_persona: {
        name: "María",
        age: "28-34",
        pain_points: ["overwhelmed by financial jargon", "afraid of investing"],
        goals: ["build emergency fund", "start investing", "buy first home"],
      },
      "content.pillars": [
        "investing basics",
        "budgeting tips",
      ...
      offer_details: {
        offer_title: "Finance Starter Kit",
        offer_type: "digital_product",
        offer_price: 29,
        offer_audience: "beginner investors",
        value_proposition: "5-step roadmap to start investing with €50/month",
      },
      offer_defined: true,
  ...
  await upsert(
    "brain_facts",
    buildFacts(bobProjId, {
      "identity.niche": {
        niche: "Productivity and minimalism",
        subniche: "Digital minimalism",
        target_audience: "Overwhelmed professionals",
        unique_angle: "Less is more approach to productivity",
      },
      "identity.primary_goal":
        "Start posting consistently and build an audience from zero",
  ...
  await upsert(
    "brain_facts",
    buildFacts(carolProjId, {
      "identity.niche": {
        niche: "Fitness",
        subniche: "Functional training and nutrition",
        target_audience: "Adults 30-50 wanting sustainable fitness",
        unique_angle: "Science-based approach that fits busy schedules",
      },
      "identity.primary_goal":
        "Scale to 100k subscribers and launch online coaching program",
      "identity.main_blocker":
        "Hard to repurpose content across platforms efficiently",
      "identity.flow_variant": "B_has_presence",
      "identity.north_star":
        "Help 1 million people adopt a sustainable, healthy lifestyle",
      ...
      current_phase: "F5",
      vision_statement:
        "Build the leading Spanish-language fitness education platform",
      creative_purpose:
        "Prove that fitness can be sustainable without extreme sacrifices",
      ...
      media_kit: {
        ...
        media_kit_url: "https://carolfit.com/media-kit",
        updated_at: agoDate(10),
      },
      offer_details: {
        offer_title: "FitPro 12-Week Program",
        offer_type: "course",
        offer_price: 197,
        offer_audience: "busy adults",
        value_proposition:
          "Transform your fitness in 12 weeks with 30-min/day workouts",
      },
      offer_defined: true,
      last_mission_date: daysDate(0),
      streak_freezes_available: 2,
  ...
  const signals = [
    ...
    buildSignal(
      aliceProjId,
      "scrape_completed",
      { platform: "instagram", followers: 12400, avg_er: 3.2, posts_7d: 4 },
      "scrape",
      1,
    ),
    buildSignal(
      aliceProjId,
      "scrape_completed",
      { platform: "instagram", followers: 11800, avg_er: 3.0, posts_7d: 3 },
      "scrape",
      8,
    ),
    buildSignal(
      ...
      { value: 12400, platform: "instagram" },
      "scrape",
      1,
    ),
    buildSignal(
      aliceProjId,
      "growth.acceleration",
      { pct: 5.1, delta: 600, platform: "instagram" },
      "scrape",
      1,
    ),
    buildSignal(
    ...
    buildSignal(
      ...
      { post_id: "IG_POST_123", multiplier: 3.8, platform: "instagram" },
      "scrape",
      5,
    ),
    buildSignal(
      aliceProjId,
      "persona_defined",
      { mission: "AUDIENCE_PERSONA_V1" },
      "mission",
      20,
    ),
    buildSignal(
      aliceProjId,
      "content_style_defined",
      { mission: "CONTENT_COMFORT_STYLE_V1" },
      "mission",
      25,
    ),
    buildSignal(
    ...
    buildSignal(
      aliceProjId,
      "content_published",
      { platform: "instagram", content_type: "reel", scheduled_at: agoDate(3) },
      "calendar",
      3,
    ),
    buildSignal(
    ...
    buildSignal(
      carolProjId,
      "scrape_completed",
      { platform: "youtube", followers: 67000, avg_er: 4.8, posts_7d: 3 },
      "scrape",
      1,
    ),
    buildSignal(
      carolProjId,
      "scrape_completed",
      { platform: "instagram", followers: 34000, avg_er: 5.1, posts_7d: 5 },
      "scrape",
      1,
    ),
    buildSignal(
      carolProjId,
      "scrape_completed",
      { platform: "youtube", followers: 64000, avg_er: 4.6, posts_7d: 2 },
      "scrape",
      8,
    ),
    buildSignal(
    ...
    buildSignal(
      carolProjId,
      "growth.acceleration",
      { pct: 4.7, delta: 3000, platform: "youtube" },
      "scrape",
      1,
    ),
    buildSignal(
    ...
    buildSignal(
      carolProjId,
      "content_perf.viral_spike",
      { post_id: "YT_VIDS_456", multiplier: 5.2, platform: "youtube" },
      "scrape",
      3,
    ),
    buildSignal(
    ...
    buildSignal(
      carolProjId,
      "content_published",
      { platform: "youtube", content_type: "video", scheduled_at: agoDate(0) },
      "calendar",
      0,
    ),
    buildSignal(
      carolProjId,
      "content_published",
      { platform: "instagram", content_type: "reel", scheduled_at: agoDate(1) },
      "calendar",
      1,
    ),
    buildSignal(
      carolProjId,
      "content_published",
      { platform: "youtube", content_type: "video", scheduled_at: agoDate(2) },
      "calendar",
      2,
    ),
    buildSignal(
      ...
      { partner: "fitcoach_maria", platform: "youtube", views: 85000 },
      "mission",
      20,
    ),
    buildSignal(
  ...
  await ins("brain_snapshots", [
    {
      ...
      phase_code: "F0",
      facts_snapshot: {
        "identity.niche": "Personal finance",
        "platforms.focus": { platform: "instagram" },
      },
    ...
    },
    {
      ...
      phase_code: "F0",
      facts_snapshot: {
        "identity.niche": "Fitness",
        "platforms.focus": { platform: "youtube" },
      },
      trigger_source: "onboarding_completed",
  ...
  await ins("core_scrape_runs", [
    {
      ...
      platform: "instagram",
      handle: "@alicefinance",
      period_start: agoDate(8),
      period_end: agoDate(8),
      raw_json: { followers: 11800 },
      normalized_json: { followers_total: 11800, avg_er_estimated: 0.03 },
    ...
    },
    {
      ...
      platform: "instagram",
      handle: "@alicefinance",
      period_start: agoDate(1),
      period_end: agoDate(1),
      raw_json: { followers: 12400 },
      normalized_json: { followers_total: 12400, avg_er_estimated: 0.032 },
    ...
    },
    {
      ...
      period_start: agoDate(1),
      period_end: agoDate(1),
      raw_json: { subscribers: 67000 },
      normalized_json: { followers_total: 67000, avg_er_estimated: 0.048 },
    ...
    },
    {
      ...
      period_start: agoDate(1),
      period_end: agoDate(1),
      raw_json: { followers: 34000 },
      normalized_json: { followers_total: 34000, avg_er_estimated: 0.051 },
  ...
  await ins("core_phase_runs", [
    {
      ...
      confidence: 0.88,
      reason_summary:
        "F3 gates pass: streak 18d, ER 3.2%, winner_format confirmed",
    ...
    },
    {
      ...
      gates_json: { F1: false },
      confidence: 0.95,
      reason_summary: "F0: no publications, no platform metrics yet",
    ...
    },
    {
      ...
      confidence: 0.92,
      reason_summary:
        "F5 gates pass: media_kit defined, 5+ batches, 1 collaboration",
  ...
  await ins("core_policy_runs", [
    {
      ...
      output_json: {
        active_mode: "monetize",
        mission_queue_delta: ["DEFINE_OFFER_V1"],
        rationale:
          "F3+ with monetization signals — focus on product definition",
      },
    },
    {
      project_id: bobProjId,
      ...
      output_json: {
        active_mode: "test",
        mission_queue_delta: ["VISION_PURPOSE_MOODBOARD_V1"],
        rationale: "Core Flow gate: first mission in queue",
      },
    },
    {
      project_id: carolProjId,
      ...
      output_json: {
        active_mode: "scale",
        mission_queue_delta: ["CONTENT_BATCH_3D_V1", "AUDIENCE_PERSONA_V1"],
        rationale:
          "F5 scale mode: batch execution and satellite platform expansion",
      },
    },
  ]);
  ...
  const aliceMissions = [
    ...
    buildMissionInstance(aliceProjId, "DEFINE_OFFER_V1", "waiting_input", null),
  ];

  // Bob — VISION in waiting_input
  const bobMissions = [
    buildMissionInstance(
      bobProjId,
      "VISION_PURPOSE_MOODBOARD_V1",
  ...
  await admin
    ...
    .delete()
    .eq("project_id", aliceProjId);
  await admin
    .from("core_calendar_items")
    .delete()
    .eq("project_id", carolProjId);
  ...
  const aliceOut1 = uuid();
  const aliceOut2 = uuid();
  const carolOut1 = uuid();
  const carolOut2 = uuid();
  const carolOut3 = uuid();
  ...
  await ins("core_outputs", [
    // Alice
    {
      ...
      platform: "instagram",
      content: {
        hook: "5 investment mistakes most 30-year-olds make 💸",
        caption:
          "Are you making these? Save this before you invest a single euro...",
        slides: [
          'Mistake 1: Waiting for the "perfect moment"',
          "Mistake 2: Not diversifying",
          "Mistake 3: Ignoring fees",
        ],
        hashtags: ["#finanzaspersonales", "#inversiones", "#millennialmoney"],
        cta: "Share with a friend who needs this!",
      },
      status: "published",
      idempotency_key: `output_alice_carousel_1`,
    },
    {
      id: aliceOut2,
      project_id: aliceProjId,
      ...
      platform: "instagram",
      content: {
        hook: "How I saved €1000 in 30 days on a €2000 salary",
        script_outline: [
          "Intro: my situation",
          "Rule 1: 50/30/20",
          "Rule 2: No impulse buys",
          "Result: €1,127 saved",
        ],
        cta: "Comment your monthly savings goal 👇",
        hashtags: ["#ahorro", "#finanzas", "#goals"],
      },
      status: "approved",
      idempotency_key: `output_alice_reel_1`,
    },
    // Carol
    {
      id: carolOut1,
      ...
      content: {
        title:
          "The 30-Minute Full Body Workout That Actually Works (No Equipment)",
        description:
          "Science-backed functional training you can do anywhere...",
        talking_points: [
          "Why traditional cardio fails",
          "The 4-movement pattern",
          "Progressive overload at home",
          "Real results from followers",
        ],
        cta: "Subscribe for weekly workouts",
        tags: ["fitness", "workout", "noequipment"],
      },
      status: "published",
      idempotency_key: `output_carol_video_1`,
    },
    {
      id: carolOut2,
      project_id: carolProjId,
      ...
      platform: "instagram",
      content: {
        hook: "This 10-minute morning routine changed my clients' lives 🔥",
        script_outline: [
          "Min 1: breathwork",
          "Min 2-5: mobility",
          "Min 6-10: activation",
        ],
        hashtags: ["#fitness", "#morningroutine", "#saludable", "#habitos"],
      },
      status: "published",
      idempotency_key: `output_carol_reel_1`,
    },
    {
      id: carolOut3,
      project_id: carolProjId,
      ...
      platform: "youtube",
      content: {
        title: "Nutrition Myths That Are Destroying Your Progress",
        description: "Evidence-based nutrition guide...",
        talking_points: [
      ...
      status: "draft",
      idempotency_key: `output_carol_video_2`,
    },
  ]);
  ...
  await ins("core_calendar_items", [
    // Alice — past published + upcoming scheduled
    {
      ...
      published_at: ago(5),
      content_type: "carousel",
      idempotency_key: `cal_alice_ig_${agoDate(5)}_carousel`,
    },
    {
      project_id: aliceProjId,
      output_id: aliceOut2,
      ...
      published_at: ago(3),
      content_type: "reel",
      idempotency_key: `cal_alice_ig_${agoDate(3)}_reel`,
    },
    {
      project_id: aliceProjId,
      platform: "instagram",
      ...
      published_at: ago(1),
      content_type: "carousel",
      idempotency_key: `cal_alice_ig_${agoDate(1)}_carousel`,
    },
    {
      project_id: aliceProjId,
      platform: "instagram",
      ...
      state: "scheduled",
      content_type: "reel",
      idempotency_key: `cal_alice_ig_${daysDate(1)}_reel`,
    },
    {
      project_id: aliceProjId,
      platform: "instagram",
      ...
      state: "scheduled",
      content_type: "carousel",
      idempotency_key: `cal_alice_ig_${daysDate(3)}_carousel`,
    },
    {
      project_id: aliceProjId,
      platform: "instagram",
      ...
      state: "draft",
      content_type: "reel",
      idempotency_key: `cal_alice_ig_${daysDate(5)}_reel`,
    },
    {
      project_id: aliceProjId,
      platform: "instagram",
      ...
      state: "draft",
      content_type: "carousel",
      idempotency_key: `cal_alice_ig_${daysDate(7)}_carousel`,
    },
    // Carol — YouTube + Instagram multi-platform
    {
      ...
      published_at: ago(2),
      content_type: "video",
      idempotency_key: `cal_carol_yt_${agoDate(2)}_video`,
    },
    {
      project_id: carolProjId,
      output_id: carolOut2,
      ...
      published_at: ago(1),
      content_type: "reel",
      idempotency_key: `cal_carol_ig_${agoDate(1)}_reel`,
    },
    {
      project_id: carolProjId,
      platform: "youtube",
      ...
      state: "scheduled",
      content_type: "video",
      idempotency_key: `cal_carol_yt_${daysDate(2)}_video`,
    },
    {
      project_id: carolProjId,
      platform: "instagram",
      ...
      state: "scheduled",
      content_type: "reel",
      idempotency_key: `cal_carol_ig_${daysDate(2)}_reel`,
    },
    {
      project_id: carolProjId,
      output_id: carolOut3,
      ...
      state: "draft",
      content_type: "video",
      idempotency_key: `cal_carol_yt_${daysDate(4)}_video`,
    },
    {
      project_id: carolProjId,
      platform: "instagram",
      ...
      state: "draft",
      content_type: "reel",
      idempotency_key: `cal_carol_ig_${daysDate(4)}_reel`,
    },
    {
      project_id: carolProjId,
      platform: "youtube",
      ...
      state: "draft",
      content_type: "video",
      idempotency_key: `cal_carol_yt_${daysDate(6)}_video`,
    },
    // Bob — one draft item
    {
      project_id: bobProjId,
      ...
      state: "draft",
      content_type: "post",
      idempotency_key: `cal_bob_ig_${daysDate(3)}_post`,
    },
  ]);

  // ── 22. Monetization products ────────────────────────────────────────────
  log("22. Monetization products");
  const aliceProd1 = uuid();
  const aliceProd2 = uuid();
  const aliceProd3 = uuid();
  const carolProd1 = uuid();
  const carolProd2 = uuid();
  const carolProd3 = uuid();
  const carolProd4 = uuid();
  const carolProd5 = uuid();
  ...
  await ins("monetization_products", [
    // Alice — 3 products
    {
      id: aliceProd1,
      project_id: aliceProjId,
      name: "Finance Starter Kit",
      type: "digital_product",
      price_amount: 29,
      price_currency: "EUR",
      status: "active",
      platform: "gumroad",
      url: "https://gumroad.com/l/finance-starter-alice",
      notes: "5-step investing roadmap PDF + spreadsheet",
    },
    {
      id: aliceProd2,
      project_id: aliceProjId,
      name: "1:1 Finance Coaching",
      type: "service",
      price_amount: 150,
      price_currency: "EUR",
      status: "active",
      platform: "calendly",
      url: "https://calendly.com/alicefinance/coaching",
      notes: "60-min personalized financial planning session",
    },
    {
      id: aliceProd3,
      project_id: aliceProjId,
      ...
      platform: "patreon",
      url: null,
      notes: "Monthly community — paused while building content",
    },
    // Carol — 5 products
    {
      id: carolProd1,
      project_id: carolProjId,
      name: "FitPro 12-Week Program",
      type: "course",
      price_amount: 197,
      price_currency: "EUR",
      status: "active",
      platform: "teachable",
      url: "https://carolfit.teachable.com/p/fitpro",
      notes: "Main flagship product",
    },
    {
      id: carolProd2,
      project_id: carolProjId,
      name: "Elite Coaching (Monthly)",
      type: "service",
      price_amount: 500,
      price_currency: "EUR",
      status: "active",
      platform: "stripe",
      url: "https://buy.stripe.com/carolfit-coaching",
      notes: "4 sessions/month + WhatsApp support",
    },
    {
      id: carolProd3,
      project_id: carolProjId,
      name: "FitStart 4-Week Plan",
      type: "digital_product",
      price_amount: 47,
      price_currency: "EUR",
      status: "active",
      platform: "gumroad",
      url: "https://gumroad.com/l/fitstart-carol",
      notes: "Entry-level program for beginners",
    },
    {
      id: carolProd4,
      project_id: carolProjId,
      name: "Gymshark Affiliate",
      type: "affiliate",
      price_amount: null,
      price_currency: "EUR",
      status: "active",
      platform: "gymshark",
      url: "https://gymshark.com?ref=carolfit",
      notes: "10% commission on referred sales",
    },
    {
      id: carolProd5,
      project_id: carolProjId,
      name: "Protein Brand Sponsorship",
      type: "brand_deal",
      price_amount: 2000,
      price_currency: "EUR",
      status: "archived",
  ...
  await ins("monetization_events", [
    ...
    {
      project_id: aliceProjId,
      product_id: aliceProd1,
      event_type: "sale",
      amount: 29,
      currency: "EUR",
      source: "manual",
      event_date: agoDate(25),
      note: "Launched to email list",
    },
    {
      project_id: aliceProjId,
      product_id: aliceProd1,
    ...
    },
    {
      ...
      currency: "EUR",
      source: "manual",
      event_date: agoDate(10),
      note: "Viral reel drove traffic",
    },
    {
      project_id: aliceProjId,
      product_id: aliceProd1,
    ...
    },
    {
      ...
      currency: "EUR",
      source: "manual",
      event_date: agoDate(2),
      note: null,
    },
    {
      project_id: aliceProjId,
      product_id: aliceProd2,
      event_type: "sale",
      amount: 150,
      currency: "EUR",
      source: "manual",
      event_date: agoDate(18),
      note: "DM lead converted",
    },
    {
      project_id: aliceProjId,
      product_id: aliceProd2,
      ...
      currency: "EUR",
      source: "manual",
      event_date: agoDate(8),
      note: null,
    },
    {
      project_id: aliceProjId,
      product_id: aliceProd3,
      event_type: "subscription_start",
      amount: 9,
      currency: "EUR",
      source: "manual",
      event_date: agoDate(12),
      note: "Early adopter",
    },
    {
      project_id: aliceProjId,
      ...
      currency: "EUR",
      source: "manual",
      event_date: agoDate(4),
      note: "Paused community",
    },
    {
      project_id: aliceProjId,
      product_id: null,
      ...
      currency: "EUR",
      source: "manual",
      event_date: agoDate(3),
      note: "5 DM inquiries about coaching this week",
    },
    {
      project_id: aliceProjId,
      product_id: null,
      ...
      currency: "EUR",
      source: "llm_inferred",
      event_date: agoDate(1),
      note: "Link clicks from bio",
    },

    // Carol events — major revenue, multiple products
    {
      project_id: carolProjId,
      product_id: carolProd1,
      event_type: "sale",
      amount: 197,
      currency: "EUR",
      source: "stripe_webhook",
      event_date: agoDate(30),
      note: "Launch day",
    },
    {
      project_id: carolProjId,
    ...
    },
    {
      ...
      currency: "EUR",
      source: "stripe_webhook",
      event_date: agoDate(24),
      note: "Requested within 48h",
    },
    {
      project_id: carolProjId,
      product_id: carolProd2,
      event_type: "subscription_start",
      amount: 500,
      currency: "EUR",
      source: "stripe_webhook",
      event_date: agoDate(60),
      note: "Client 1",
    },
    {
      project_id: carolProjId,
    ...
    },
    {
      ...
      source: "stripe_webhook",
      event_date: agoDate(15),
      note: "Client 1 cancelled",
    },
    {
      project_id: carolProjId,
      product_id: carolProd3,
      event_type: "sale",
      amount: 47,
      currency: "EUR",
      source: "manual",
    ...
    {
      project_id: carolProjId,
      product_id: carolProd4,
      event_type: "sale",
      amount: 85,
      currency: "EUR",
      source: "llm_inferred",
      event_date: agoDate(15),
      note: "Affiliate commission estimate",
    },
    {
      project_id: carolProjId,
      product_id: carolProd4,
    ...
    {
      project_id: carolProjId,
      product_id: carolProd5,
      event_type: "sale",
      amount: 2000,
      currency: "EUR",
      source: "manual",
      event_date: agoDate(40),
      note: "Q1 brand deal payment",
    },
    {
      project_id: carolProjId,
      product_id: null,
      ...
      source: "manual",
      event_date: agoDate(2),
      note: "3 brand collaboration inquiries",
    },
  ]);
  ...
  await ins("pulse_runs", [
    // Alice — 2 pulse runs
    {
      ...
      missions_assigned: 1,
      summary:
        "Strong week: 4 posts, ER above 3%. Viral spike on finance carousel.",
      action_items: [
        {
          priority: "high",
          category: "content",
          action:
            "Repurpose your viral carousel into a Reel with the same hook — publish within 48h",
          reasoning: "Viral content compounds when cross-posted quickly",
        },
        {
          priority: "medium",
          category: "monetization",
          action:
            "Add a link to your Finance Starter Kit in your bio and next 2 captions",
          reasoning: "You have 5 DM inquiries — reduce friction to purchase",
        },
        {
          priority: "low",
          category: "growth",
          action: "Reply to every comment on your top post this week",
          reasoning: "ER is 3.2% — push it above 4% to hit F4 gate",
        },
      ],
      started_at: ago(1),
      completed_at: ago(1),
      ...
      action_items: [
        {
          ...
          category: "monetization",
          action:
            "Complete your DEFINE_OFFER mission — you have an audience ready to buy",
          reasoning: "Offer defined fact missing; monetize mode blocked",
        },
        {
          priority: "medium",
          category: "consistency",
          action: "Schedule next 7 days of content in one batch session",
          reasoning: "Batch creation reduces burnout and improves consistency",
        },
      ],
      started_at: ago(2),
      completed_at: ago(2),
    ...
    // Bob — first pulse (minimal data)
    {
      ...
      missions_assigned: 1,
      summary:
        "Getting started! Complete your first mission to unlock more insights.",
      action_items: [
        {
          priority: "high",
          category: "missions",
          action:
            "Complete the Vision & Purpose mission — it takes 10 minutes and unlocks everything",
          reasoning: "Core Flow gate: no missions completed yet",
        },
      ],
      started_at: ago(1),
      completed_at: ago(1),
    ...
    // Carol — 2 pulse runs
    {
      ...
      triggered_by: "schedule",
      phase_code: "F5",
      signals_collected: 28,
      events_detected: 4,
      missions_assigned: 2,
      summary:
        "Exceptional week: YouTube video viral spike (5.2x). Instagram engagement strong. Revenue on track.",
      action_items: [
        {
          priority: "high",
          category: "content",
          action:
            'Create a Part 2 of your viral "30-Minute Workout" video — strike while iron is hot',
          reasoning:
            "Viral multiplier 5.2x — algorithm is boosting your channel",
        },
        {
          priority: "high",
          category: "monetization",
          action:
            "Add FitPro CTA to your viral video description and pin a comment",
          reasoning:
            "45k views is a conversion opportunity — add friction-free link",
        },
        {
          priority: "medium",
          category: "growth",
          action:
            "Respond to top 20 comments on viral video to boost ER for algorithm",
          reasoning: "YouTube rewards comment engagement in first 48h",
        },
        {
          priority: "low",
          category: "platforms",
          action: "Repurpose viral video into 3 Instagram Reels this week",
          reasoning: "F6 gate requires ≥10 repurposed items/month",
        },
      ],
      started_at: ago(0),
      completed_at: ago(0),
    },
    {
      ...
      missions_assigned: 1,
      summary:
        "Strong consistency. Revenue growing. Focus on cross-platform repurposing to hit F6.",
      action_items: [
        {
          priority: "high",
          category: "growth",
          action:
            "Set up systematic repurposing: 1 YouTube video → 3 Reels → 5 Stories each week",
          reasoning:
            "F6 gate requires ≥10 repurposed items/month — currently at 4",
        },
        {
          priority: "medium",
          category: "monetization",
          action: "Contact 3 new brand collaboration opportunities this week",
          reasoning:
            "F5 gate requires ≥1 successful collaboration — you met it; F6 needs more",
        },
      ],
      started_at: ago(1),
      completed_at: ago(1),
  ...
  await ins("notifications", [
    // Alice — mix of read and unread
    {
      ...
      body: "Your carousel was published on instagram.",
      delivery_channel: "in_app",
      sent_at: ago(1),
      read_at: ago(1),
      metadata_json: { platform: "instagram", content_type: "carousel" },
    },
    {
      project_id: aliceProjId,
      user_id: alice.id,
      type: "publish_success",
      title: "Content published",
      body: "Your reel was published on instagram.",
      delivery_channel: "in_app",
      sent_at: ago(3),
      read_at: ago(3),
      metadata_json: { platform: "instagram", content_type: "reel" },
    },
    {
      project_id: aliceProjId,
      user_id: alice.id,
      ...
      body: "3 missions · 4 posts · +600 followers",
    ...
    },
    {
      ...
      body: 'Your mission "define offer" is waiting for your response.',
      delivery_channel: "email",
      sent_at: ago(0),
      read_at: null,
      metadata_json: {
        mission_instance_id: aliceMissions[8].id,
        template_code: "DEFINE_OFFER_V1",
      },
    },
    {
      project_id: aliceProjId,
      user_id: alice.id,
      type: "capability_followup",
      title: "You're ready for monetize mode!",
      body: "Your ER hit 3.2% and you have 12k followers. Time to define your first offer.",
      delivery_channel: "in_app",
      sent_at: ago(5),
      read_at: null,
      metadata_json: {},
    ...
    // Bob — unread mission reminder
    {
      ...
      type: "mission_reminder",
      title: "Mission waiting for your input",
      body: 'Your mission "vision purpose moodboard" is waiting for your response.',
      delivery_channel: "email",
      sent_at: ago(1),
      read_at: null,
      metadata_json: {
        mission_instance_id: bobMissions[0].id,
    ...
    // Carol — multiple notifications
    {
      ...
      type: "publish_success",
      title: "Content published",
      body: "Your video was published on youtube.",
      delivery_channel: "in_app",
      sent_at: ago(2),
      read_at: ago(2),
      metadata_json: { platform: "youtube", content_type: "video" },
    },
    {
      project_id: carolProjId,
      user_id: carol.id,
    ...
    },
    {
      ...
      type: "weekly_digest",
      title: "Your weekly growth summary",
      body: "5 missions · 7 posts · +3000 followers",
      delivery_channel: "email",
      sent_at: ago(7),
      read_at: ago(6),
      metadata_json: {
    ...
    },
    {
      ...
      body: "Your YouTube video got 5.2x your average views. Strike while hot — create Part 2.",
      delivery_channel: "in_app",
      sent_at: ago(3),
      read_at: null,
      metadata_json: { signal: "viral_spike", platform: "youtube" },
    },
  ]);
  ...
  await upsert(
    ...
    [
      ...
      {
        stripe_event_id: `evt_seed_carol_invoice_1`,
        event_type: "invoice.paid",
        data: { object: { subscription: carolSubId } },
        processed: true,
      },
    ],
    "stripe_event_id",
  ...
  try {
    ...
    console.log(
      "  ↳ core_gamification table not found — skipping (gamification on projects table)",
    );
  }
  ...
  console.log(
    "  alice@test.glimad.com  Seedpass123!  (F3, monetize, PRO)   — fully loaded",
  );
  console.log(
    "  bob@test.glimad.com    Seedpass123!  (F0, test, BASE)      — just started",
  );
  console.log(
    "  carol@test.glimad.com  Seedpass123!  (F5, scale, ELITE)    — power user",
  );
}
