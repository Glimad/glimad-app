...
async function cleanupProject(projectId: string) {
  ...
  await admin.from('onboarding_sessions').delete().eq('converted_to_user_id', projectId.slice(0, 0)) // noop
...
async function sendWebhookEvent(eventType: string, eventData: object): Promise<Response> {
...
async function testCheckout(token: string) {
  ...
  for (const plan_code of ['BASE', 'PRO', 'ELITE']) {
    ...
    ok(`${plan_code} url is present`, typeof body.url === 'string' && body.url.startsWith('https://'),
      `url=${body.url}`)
    ok(`${plan_code} url is Stripe checkout`, body.url?.includes('checkout.stripe.com'),
      `url=${body.url}`)
  }
}
...
async function testCheckoutSessionCompleted(userId: string, projectId: string) {
  ...
  const sessionObject = {
    ...
    metadata: { user_id: userId, plan_code: 'BASE' },
  }

  const res = await sendWebhookEvent('checkout.session.completed', sessionObject)
  ...
  // ── core_subscriptions ────────────────────────────────────────────────────
  const { data: sub } = await admin.from('core_subscriptions')
    .select('*').eq('stripe_subscription_id', subscription.id).single()
  ok('core_subscriptions row created', !!sub, 'row missing')
  ok('subscription status = active', sub?.status === 'active', `status=${sub?.status}`)
  ok('subscription plan_code = BASE', sub?.plan_code === 'BASE', `plan=${sub?.plan_code}`)
  ok('subscription user_id set', sub?.user_id === userId, `user_id=${sub?.user_id}`)
  ok('subscription project_id set', sub?.project_id === projectId, `project_id=${sub?.project_id}`)
  ...
  // ── core_access_grants ────────────────────────────────────────────────────
  const { data: grant } = await admin.from('core_access_grants')
    .select('*').eq('reference_id', subscription.id).eq('status', 'active').single()
  ok('core_access_grants row created', !!grant, 'row missing')
  ok('access grant source = subscription', grant?.source === 'subscription', `source=${grant?.source}`)
  ok('access grant user_id set', grant?.user_id === userId, `user_id=${grant?.user_id}`)
  ...
  ok('wallet plan_code = BASE', wallet?.plan_code === 'BASE', `plan=${wallet?.plan_code}`)
  ok('wallet status = active', wallet?.status === 'active', `status=${wallet?.status}`)
  ok('wallet allowance_llm_balance = 2000', wallet?.allowance_llm_balance === 2000,
    `balance=${wallet?.allowance_llm_balance}`)
  ok('wallet premium_credits_balance = 500', wallet?.premium_credits_balance === 500,
    `balance=${wallet?.premium_credits_balance}`)

  // ── core_ledger credit entry ──────────────────────────────────────────────
  const { data: ledger } = await admin.from('core_ledger')
    .select('*').eq('project_id', projectId).eq('kind', 'credit').eq('reason_key', 'PLAN_MONTHLY_GRANT').single()
  ok('core_ledger PLAN_MONTHLY_GRANT entry created', !!ledger, 'row missing')
  ok('ledger amount_allowance = 2000', ledger?.amount_allowance === 2000, `got ${ledger?.amount_allowance}`)
  ok('ledger amount_premium = 500', ledger?.amount_premium === 500, `got ${ledger?.amount_premium}`)
  ...
  ok('brain_facts: niche_raw written', factMap['niche_raw'] !== undefined,
    `got=${JSON.stringify(factMap['niche_raw'])}`)
  ok('brain_facts: primary_goal written', factMap['primary_goal'] !== undefined)
  ok('brain_facts: main_blocker written', factMap['main_blocker'] !== undefined)
  ok('brain_facts: on_camera_comfort written', factMap['on_camera_comfort'] !== undefined)
  ok('brain_facts: current_platforms written', factMap['current_platforms'] !== undefined)
  ok('brain_facts: focus_platform_handle written', factMap['focus_platform_handle'] !== undefined)

  // ── brain_signals from onboarding ────────────────────────────────────────
  const { data: signals } = await admin.from('brain_signals')
    .select('signal_key').eq('project_id', projectId)
  const sigKeys = new Set((signals ?? []).map((s: { signal_key: string }) => s.signal_key))
  ok('brain_signals: onboarding_completed written', sigKeys.has('onboarding_completed'),
    `signals=${Array.from(sigKeys).join(',')}`)
  ok('brain_signals: platform_declared written', sigKeys.has('platform_declared'))

  // ── user_preferences ─────────────────────────────────────────────────────
  const { data: prefs } = await admin.from('user_preferences')
    .select('*').eq('project_id', projectId).single()
  ok('user_preferences row written', !!prefs, 'row missing')
  ok('user_preferences face_visibility set', ['yes', 'no', 'maybe'].includes(prefs?.face_visibility ?? ''),
    `face_visibility=${prefs?.face_visibility}`)
  ok('user_preferences availability_hours_week set', (prefs?.availability_hours_week ?? 0) > 0,
  ...
  ok('core_phase_runs entry created (phase engine ran)', !!phaseRun?.phase_code,
    'no core_phase_runs row')
  ok('phase_code is valid F-code', /^F[0-7]$/.test(phaseRun?.phase_code ?? ''),
    `phase_code=${phaseRun?.phase_code}`)

  // ── stripe_events: event saved as processed ────────────────────────────────
  const { data: stripeEvent } = await admin.from('stripe_events')
    .select('processed').eq('event_type', 'checkout.session.completed')
    .order('created_at', { ascending: false }).limit(1).single()
  ok('stripe_events: event saved as processed', stripeEvent?.processed === true,
...
async function testInvoicePaid(userId: string, projectId: string, subscriptionId: string) {
  console.log('\n[3] invoice.paid → renewal credits granted, period updated')

  const admin = createClient(SUPABASE_URL, SERVICE_KEY, { auth: { persistSession: false } })

  // Get current ledger count before renewal
  const { count: beforeCount } = await admin.from('core_ledger')
  ...
  ok('invoice.paid webhook returns 200', res.status === 200, `got ${res.status}`)

  await new Promise(r => setTimeout(r, 1500))

  // Ledger should have a new credit entry for the renewal
  const { data: renewalLedger } = await admin.from('core_ledger')
    .select('*').eq('project_id', projectId)
    .eq('idempotency_key', `invoice_${invoiceId}_grant`).single()
  ok('core_ledger: renewal grant entry created', !!renewalLedger, 'row missing')
  ok('renewal grant amount_allowance = 2000', renewalLedger?.amount_allowance === 2000,
    `got=${renewalLedger?.amount_allowance}`)
  ok('renewal grant amount_premium = 500', renewalLedger?.amount_premium === 500,
  ...
  ok('invoice.paid idempotency: second call returns 200', res2.status === 200, `got ${res2.status}`)
  await new Promise(r => setTimeout(r, 1000))

  const { count: afterCount } = await admin.from('core_ledger')
    .select('*', { count: 'exact', head: true })
    .eq('project_id', projectId)
    .eq('idempotency_key', `invoice_${invoiceId}_grant`)
  ok('invoice.paid idempotency: no duplicate ledger row', afterCount === 1,
    `count=${afterCount}`)
}
...
async function testPaymentFailed(projectId: string, subscriptionId: string) {
  ...
  ok('invoice.payment_failed webhook returns 200', res.status === 200, `got ${res.status}`)

  await new Promise(r => setTimeout(r, 1500))

  const { data: sub } = await admin.from('core_subscriptions')
    .select('status').eq('stripe_subscription_id', subscriptionId).single()
  ok('subscription status = past_due', sub?.status === 'past_due', `status=${sub?.status}`)

  const { data: wallet } = await admin.from('core_wallets')
    .select('status').eq('project_id', projectId).single()
  ok('wallet status = past_due', wallet?.status === 'past_due', `status=${wallet?.status}`)

  // Restore wallet to active for subsequent tests
  await admin.from('core_wallets').update({ status: 'active' }).eq('project_id', projectId)
  await admin.from('core_subscriptions')
    .update({ status: 'active' }).eq('stripe_subscription_id', subscriptionId)
}
...
async function testChargeRefunded(projectId: string, subscriptionId: string) {
  ...
  ok('charge.refunded webhook returns 200', res.status === 200, `got ${res.status}`)

  await new Promise(r => setTimeout(r, 1500))

  const { data: sub } = await admin.from('core_subscriptions')
    .select('status').eq('stripe_subscription_id', subscriptionId).single()
  ok('subscription status = canceled', sub?.status === 'canceled', `status=${sub?.status}`)

  const { data: grant } = await admin.from('core_access_grants')
    .select('status, revoked_at').eq('reference_id', subscriptionId).single()
  ok('access grant status = revoked', grant?.status === 'revoked', `status=${grant?.status}`)
  ok('access grant revoked_at set', !!grant?.revoked_at, 'revoked_at missing')

  const { data: wallet } = await admin.from('core_wallets')
    .select('status').eq('project_id', projectId).single()
  ok('wallet status = locked', wallet?.status === 'locked', `status=${wallet?.status}`)

  const { data: refundLedger } = await admin.from('core_ledger')
    .select('*').eq('project_id', projectId).eq('idempotency_key', `refund_${chargeId}`).single()
  ok('core_ledger: refund entry created', !!refundLedger, 'row missing')
  ok('refund ledger ref_type = refund', refundLedger?.ref_type === 'refund',
    `ref_type=${refundLedger?.ref_type}`)
  ok('refund ledger reason_key = REFUND_CREDIT', refundLedger?.reason_key === 'REFUND_CREDIT',
...
async function testSubscriptionDeleted(subscriptionId: string) {
  ...
  const res = await sendWebhookEvent('customer.subscription.deleted', subObject)
  ok('customer.subscription.deleted webhook returns 200', res.status === 200, `got ${res.status}`)
...
async function testWebhookIdempotency(userId: string, projectId: string, subscriptionId: string) {
  console.log('\n[7] Webhook idempotency — same event ID processed twice → no duplicate records')

  const admin = createClient(SUPABASE_URL, SERVICE_KEY, { auth: { persistSession: false } })

  // Get ledger count before
  const { count: ledgerBefore } = await admin.from('core_ledger')
    .select('*', { count: 'exact', head: true }).eq('project_id', projectId)
  ...
  const { count: ledgerAfter } = await admin.from('core_ledger')
    .select('*', { count: 'exact', head: true }).eq('project_id', projectId)

  // stripe_events table should have exactly 1 row for this event ID
  const { count: eventCount } = await admin.from('stripe_events')
    .select('*', { count: 'exact', head: true }).eq('stripe_event_id', fixedEventId)
  ok('stripe_events: exactly 1 row for duplicate event', eventCount === 1,
    `count=${eventCount}`)
}
...
async function main() {
  ...
  try {
    ...
    const { subscriptionId: subId } = await testCheckoutSessionCompleted(userId, projectId)
    ...
    const admin = createClient(SUPABASE_URL, SERVICE_KEY, { auth: { persistSession: false } })
    if (subscriptionId) {
      await admin.from('stripe_events').delete().eq('event_type', 'checkout.session.completed')
