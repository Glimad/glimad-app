    ...
    .upsert(
      {
        project_id: proj.id,
        plan_code: "starter",
        allowance_llm_balance: 2000,
        credits_allowance: 2000,
        premium_credits_balance: 0,
        premium_daily_cap_remaining: 0,
  ...
  await sb()
    ...
    .upsert(
      [
        ...
        {
          project_id: proj.id,
          fact_key: "audience_persona",
          value: "Mamás de 30-45",
          source: "test",
          updated_at: new Date().toISOString(),
        },
      ],
...
async function testGetPlatformContext(token: string) {
  ...
  ok(
    "caption_limit = 2200",
    data.caption_limit === 2200,
    `got: ${data.caption_limit}`,
  );
}

async function testTopics(token: string) {
  console.log("\n[2] POST /api/studio/topics returns 6 topic strings");
  const { status, data } = await post("/api/studio/topics", token, {
    content_type: "reel",
  });
  ok("status 200", status === 200, `got: ${status}`);
  ok(
    ...
    `got: ${typeof data.topics}`,
  );
  ok(
    "exactly 6 topics",
    data.topics?.length === 6,
    `got: ${data.topics?.length}`,
  );
...
async function testApproveScheduled(projectId: string, token: string) {
  ...
  ok(
    "content saved",
    (asset?.content as Record<string, unknown>)?.["hook"] === "Test hook",
  );

  const { data: cal } = await sb()
    .from("core_calendar_items")
    .select("state, scheduled_at")
    .eq("id", data.calendar_item_id)
    .single();
...
async function testUnauthorized() {
  ...
  const r2 = await fetch(`${BASE}/api/studio/generate`, {
    method: "POST",
  ...
  const r3 = await fetch(`${BASE}/api/studio/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: "{}",
  });
