...
async function getToken(): Promise<{ token: string; userId: string }> {
  ...
  if (signInErr || !data.session)
    throw new Error(`Auth failed: ${signInErr?.message}`);
...
async function testF0CoreFlowGate(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "topMission is first Core Flow mission",
    policy.topMission === "VISION_PURPOSE_MOODBOARD_V1",
    `got: ${policy.topMission}`,
  );
  ok(
    "missionQueue has exactly 1 item",
    policy.missionQueue?.length === 1,
    `got: ${policy.missionQueue?.length}`,
  );
  ok(
    "score is 100",
    policy.missionQueue?.[0]?.priorityScore === 100,
    `got: ${policy.missionQueue?.[0]?.priorityScore}`,
  );
  ok(
    "rationale mentions F0 Core Flow",
    policy.rationale?.includes("F0 Core Flow"),
    `got: ${policy.rationale}`,
  );
}

async function testF0CoreFlowPartial(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "topMission is second Core Flow mission",
    policy.topMission === "NICHE_CONFIRM_V1",
    `got: ${policy.topMission}`,
  );
  ok(
    "score is 100",
...
async function testF0AllCoreFlowDone(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "topMission is NOT a Core Flow mission",
    ![
      "VISION_PURPOSE_MOODBOARD_V1",
      "NICHE_CONFIRM_V1",
      "PLATFORM_STRATEGY_PICKER_V1",
      "PREFERENCES_CAPTURE_V1",
    ].includes(policy.topMission ?? ""),
    `got: ${policy.topMission}`,
  );
  ok(
    "missionQueue exists",
    Array.isArray(policy.missionQueue),
    `got: ${typeof policy.missionQueue}`,
  );
}

async function testInflexionBonus(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  await seedSignal(
    ...
    { multiplier: 8, video_id: "test_vid" },
    2,
  );
  ...
  const data = await runEngines(token);
  const policy = data.policy;
  const contentBatch = policy.missionQueue?.find(
    (m: { templateCode: string }) => m.templateCode === "CONTENT_BATCH_3D_V1",
  );

  ok(
    "CONTENT_BATCH_3D_V1 is in queue",
    !!contentBatch,
    `queue: ${JSON.stringify(policy.missionQueue?.map((m: { templateCode: string }) => m.templateCode))}`,
  );
  ok(
    "topMission is CONTENT_BATCH_3D_V1 (highest score due to inflexion)",
    policy.topMission === "CONTENT_BATCH_3D_V1",
    `got: ${policy.topMission}`,
  );
  // Base P2=60 + inflexion +50 + phase_rec +30 + new +20 = 160 (minus premium penalty if low credits; wallet has 500)
  ok(
    "score includes inflexion bonus (≥110)",
    (contentBatch?.priorityScore ?? 0) >= 110,
...
async function seedF3BrainState(projectId: string) {
  ...
  await seedFact(
    projectId,
    "audience_persona",
    "health-conscious 25-35 year olds",
  );
  await seedFact(
    projectId,
...
async function testPhaseRecommendationBonus(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  const defineOffer = policy.missionQueue?.find(
    (m: { templateCode: string }) => m.templateCode === "DEFINE_OFFER_V1",
  );

  ok(
    "phase is F3 or above",
    ["F3", "F4", "F5", "F6", "F7"].includes(data.phaseResult?.phase),
    `got: ${data.phaseResult?.phase} (score: ${data.phaseResult?.capabilityScore})`,
  );
  ok("CONTENT_BATCH_3D_V1 in queue", !!contentBatch);
  ok(
    "DEFINE_OFFER_V1 in queue",
    !!defineOffer,
    `queue: ${JSON.stringify(policy.missionQueue?.map((m: { templateCode: string }) => m.templateCode))}`,
  );
  ok(
    "Phase-recommended missions have bonus score (≥90)",
    (contentBatch?.priorityScore ?? 0) >= 90,
    `CONTENT_BATCH score: ${contentBatch?.priorityScore}`,
  );
}

async function testBurnoutPenalty(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  const engagementRescue = policy.missionQueue?.find(
    (m: { templateCode: string }) => m.templateCode === "ENGAGEMENT_RESCUE_V1",
  );

  ok("CONTENT_BATCH_3D_V1 in queue", !!contentBatch);
  ok(
    "reason includes burnout_penalized",
    contentBatch?.reason?.includes("burnout_penalized"),
  ...
  ok(
    "score is reduced (≤100)",
    (contentBatch?.priorityScore ?? 200) <= 100,
    `got: ${contentBatch?.priorityScore}`,
  );
}

async function testDailyLimitReached(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  const allScoresZeroOrFiltered =
    policy.missionQueue?.every(
      (m: { priorityScore: number }) => m.priorityScore === 0,
    ) ?? true;
  // All missions cost allowance credits, so all should score 0 when daily limit reached
  ok(
    "All missions score 0 when daily limit reached",
    allScoresZeroOrFiltered,
  ...
  ok(
    ...
    policy.missionQueue?.some((m: { reason: string }) =>
      m.reason?.includes("daily_limit"),
    ),
    `reasons: ${policy.missionQueue?.map((m: { reason: string }) => m.reason)}`,
  );
}

async function testNoPremiumCreditsFilter(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "CONTENT_BATCH_3D_V1 filtered out (premium mission, 0 premium credits)",
    !contentBatch,
  ...
  ok(
    "ENGAGEMENT_RESCUE_V1 still available (no premium cost)",
    !!engagementRescue,
  );
}

async function testActiveModeViralSpike(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "activeMode is scale",
    data.policy?.activeMode === "scale",
    `got: ${data.policy?.activeMode}`,
  );
  ok(
    "inflexion is viral_spike",
    data.inflexion?.type === "viral_spike",
    `got: ${data.inflexion?.type}`,
  );
}

async function testActiveModeMonetization(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "activeMode is monetize",
    data.policy?.activeMode === "monetize",
    `got: ${data.policy?.activeMode}`,
  );
  ok(
    "inflexion is monetize_ready",
    data.inflexion?.type === "monetize_ready",
    `got: ${data.inflexion?.type}`,
  );
  ok(
    "topMission is DEFINE_OFFER_V1",
    data.policy?.topMission === "DEFINE_OFFER_V1",
    `got: ${data.policy?.topMission}`,
  );
}

async function testCompletedOver30DaysBonus(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  const data = await runEngines(token);
  const policy = data.policy;
  const rescue = policy.missionQueue?.find(
    (m: { templateCode: string }) => m.templateCode === "ENGAGEMENT_RESCUE_V1",
  );

  ok(
    "ENGAGEMENT_RESCUE_V1 in queue (cooldown expired)",
    !!rescue,
    `queue: ${JSON.stringify(policy.missionQueue?.map((m: { templateCode: string }) => m.templateCode))}`,
  );
  // P1=80 + phase_rec +30 (F1 recommended) + repeat >30d +10 = 120 (no "new" bonus since it was completed)
  ok(
    "+10 repeat bonus applied (score ≥ 110)",
    (rescue?.priorityScore ?? 0) >= 110,
    `got: ${rescue?.priorityScore}`,
  );
}

async function testActiveMissionFilter(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "ENGAGEMENT_RESCUE_V1 filtered out (already running)",
    !rescue,
...
async function testCooldownFilter(
  ...
  ok(
    "ENGAGEMENT_RESCUE_V1 filtered out (within 7-day cooldown)",
    !rescue,
    `found with score: ${rescue?.priorityScore}`,
  );
}

async function testLowWalletPremiumPenalty(
  projectId: string,
  userId: string,
  token: string,
) {
  ...
  ok(
    "CONTENT_BATCH_3D_V1 still in queue (30 > 0 premium credits)",
    !!contentBatch,
    `queue: ${JSON.stringify(policy.missionQueue?.map((m: { templateCode: string }) => m.templateCode))}`,
  );
  ...
  ok(
    "score reduced by -40 penalty (< 160)",
    (contentBatch?.priorityScore ?? 200) < 160,
    `got: ${contentBatch?.priorityScore}`,
  );
}
