...
async function seedProject(db: ReturnType<typeof admin>) {
  ...
  const { data: existingProj } = await db.from('projects').select('user_id').limit(1).single()
  if (!existingProj) throw new Error('No existing projects/users in DB to borrow user_id from')
...
async function testMissingHandle(db: ReturnType<typeof admin>, projectId: string) {
  console.log('\n[1] Missing handle → missing_evidence signal')

  const result = await requestScrapeLight(db as any, projectId, 'user-1', 'instagram', '')
  ok('returns skipped_no_handle status', result.status === 'skipped_no_handle')
  ok('returns empty job_id', result.job_id === '')

  const signals = await readSignals(db as any, projectId, 1, 'missing_evidence')
  ok('appends missing_evidence signal', signals.length === 1, `got ${signals.length}`)

  const sig = signals[0]?.value as any
  ok('signal has reason=no_handle_provided', sig?.reason === 'no_handle_provided', JSON.stringify(sig))
  ok('signal has platform=instagram', sig?.platform === 'instagram', JSON.stringify(sig))
}

// ── TEST 2: job creation ──────────────────────────────────────────────────────

async function testJobCreation(db: ReturnType<typeof admin>, projectId: string, userId: string) {
  console.log('\n[2] Job creation')

  const result = await requestScrapeLight(db as any, projectId, userId, 'instagram', 'leomessi')
  ok('returns job_id', !!result.job_id, `job_id=${result.job_id}`)
  ok('status is queued', result.status === 'queued', `status=${result.status}`)

  const { data: job, error } = await db.from('core_jobs')
    .select('*').eq('job_id', result.job_id).single()
  ...
  ok('payload platform=instagram', (job?.payload_json as any)?.platform === 'instagram')
  ok('payload handle=leomessi', (job?.payload_json as any)?.handle === 'leomessi')
...
async function testRateLimitQueued(db: ReturnType<typeof admin>, projectId: string, userId: string, existingJobId: string) {
  console.log('\n[3] Rate limit — already queued job')

  const result = await requestScrapeLight(db as any, projectId, userId, 'instagram', 'leomessi')
  ok('returns same job_id (no duplicate)', result.job_id === existingJobId, `got ${result.job_id}`)

  const { data: jobs } = await db.from('core_jobs')
    .select('job_id').eq('project_id', projectId).eq('job_type', 'scrape_light')
  ok('only 1 job in DB', jobs?.length === 1, `found ${jobs?.length}`)
}

// ── TEST 4: rate limiting — done within 24h ───────────────────────────────────

async function testRateLimitDone24h(db: ReturnType<typeof admin>, projectId: string, userId: string) {
  console.log('\n[4] Rate limit — done job within 24h')

  const finishedAt = new Date(Date.now() - 60 * 60 * 1000).toISOString()
  const { data: doneJob, error } = await db.from('core_jobs').insert({
  ...
  const result = await requestScrapeLight(db as any, projectId, userId, 'tiktok', 'testhandle')
  ok('returns rate_limited status', result.status === 'rate_limited', `got ${result.status}`)
  ok('returns existing job_id', result.job_id === doneJob!.job_id, `got ${result.job_id}`)
...
async function testExecuteInstagram(db: ReturnType<typeof admin>, projectId: string, jobId: string) {
  console.log('\n[5] Execute Instagram scrape job (leomessi)')

  await executeScrapeLightJob(db as any, jobId)

  // Job state
  const { data: job } = await db.from('core_jobs')
    .select('status, finished_at, attempts').eq('job_id', jobId).single()
  ...
  // Scrape run
  const { data: run } = await db.from('core_scrape_runs')
    .select('*').eq('project_id', projectId).eq('platform', 'instagram').single()
  ...
  ok('followers_total > 0 in normalized', (norm?.followers_total ?? 0) > 0, `got ${norm?.followers_total}`)
  ok('avg_er_estimated present', norm?.avg_er_estimated !== undefined)
  ok('last_post_date present', norm?.last_post_date !== undefined)

  // platform_metrics
  const { data: metrics } = await db.from('platform_metrics')
    .select('*').eq('project_id', projectId).eq('platform', 'instagram').single()
  ok('platform_metrics row created', !!metrics)
  ok('followers_count > 0', (metrics?.followers_count ?? 0) > 0, `got ${metrics?.followers_count}`)
  ...
  const postsPerWeek = await readFact(db as any, projectId, 'posts_per_week_average')
  ok('fact: followers_total > 0', !!followers && Number(followers) > 0, `got ${followers}`)
  ok('fact: avg_engagement_rate written', er !== null, `got ${er}`)
  ok('fact: last_post_date written', lastPost !== null, `got ${lastPost}`)
  ok('fact: posts_per_week_average written', postsPerWeek !== null, `got ${postsPerWeek}`)

  // brain signals
  const completedSigs = await readSignals(db as any, projectId, 1, 'scrape_completed')
  ok('scrape_completed signal written', completedSigs.length > 0)
  ok('signal platform=instagram', (completedSigs[0]?.value as any)?.platform === 'instagram')

  const followerSigs = await readSignals(db as any, projectId, 1, 'growth.followers_total')
  ok('growth.followers_total signal written', followerSigs.length > 0)

  const erSigs = await readSignals(db as any, projectId, 1, 'engagement.avg_er_7d')
  ok('engagement.avg_er_7d signal written', erSigs.length > 0)

  const consistencySigs = await readSignals(db as any, projectId, 1, 'consistency.posts_published_7d')
  ok('consistency.posts_published_7d signal written', consistencySigs.length > 0)
}

// ── TEST 6: idempotency — re-run skips the platform call ─────────────────────

async function testIdempotency(db: ReturnType<typeof admin>, projectId: string, userId: string) {
  console.log('\n[6] Idempotency — same-day re-run skips platform API call')

  const { data: runsBefore } = await db.from('core_scrape_runs')
    .select('run_id').eq('project_id', projectId).eq('platform', 'instagram')

  const { data: dupeJob } = await db.from('core_jobs').insert({
    project_id: projectId,
    user_id: userId,
    job_type: 'scrape_light',
    status: 'queued',
  ...
  const { data: runsAfter } = await db.from('core_scrape_runs')
    .select('run_id').eq('project_id', projectId).eq('platform', 'instagram')

  ok('no new scrape_run created', runsAfter?.length === runsBefore?.length,
    `before=${runsBefore?.length}, after=${runsAfter?.length}`)

  const skippedSigs = await readSignals(db as any, projectId, 1, 'scrape_skipped')
  ok('scrape_skipped signal written', skippedSigs.length > 0)
}

// ── TEST 7: wallet debit ──────────────────────────────────────────────────────

async function testWalletDebit(db: ReturnType<typeof admin>, projectId: string) {
  console.log('\n[7] Wallet debit')

  const { data: wallet } = await db.from('core_wallets')
    .select('premium_credits_balance').eq('project_id', projectId).single()
  ...
  ok('wallet balance was debited', (wallet?.premium_credits_balance ?? 100) < 100,
    `balance=${wallet?.premium_credits_balance}`)

  // Verify ledger entries exist
  const { data: ledger } = await db.from('core_ledger')
    .select('ledger_id, amount_premium').eq('project_id', projectId)
  ok('ledger entries created', (ledger?.length ?? 0) > 0, `found ${ledger?.length}`)
...
async function testRetrySignals(db: ReturnType<typeof admin>, projectId: string, userId: string) {
  ...
  const { data: badJob } = await db.from('core_jobs').insert({
    project_id: projectId,
    user_id: userId,
    job_type: 'scrape_light',
    status: 'queued',
    ...
    cost_premium_credits: 5,
    payload_json: { platform: 'instagram', handle: 'xyzzy_this_handle_does_not_exist_9999z' },
  }).select('job_id').single()
  ...
  if (!failedWithError) {
    console.log('  ~ handle resolved — Instagram returned data, retry test skipped')
    await db.from('core_jobs').delete().eq('job_id', badJob!.job_id)
    return
  }

  const failedSigs = await readSignals(db as any, projectId, 1, 'scrape_failed')
  ok('scrape_failed signal(s) written', failedSigs.length > 0, `got ${failedSigs.length}`)
  ok('signal has attempt number', (failedSigs[0]?.value as any)?.attempt !== undefined)
  ok('signal has reason', !!(failedSigs[0]?.value as any)?.reason)

  const notifSigs = await readSignals(db as any, projectId, 1, 'user_notification')
  ok('user_notification signal written after 3 failures', notifSigs.length > 0, `got ${notifSigs.length}`)
  ok('notification type=scrape_failed_final', (notifSigs[0]?.value as any)?.type === 'scrape_failed_final')
...
async function main() {
  ...
  const hasNetwork = await fetch('https://www.instagram.com/', { signal: AbortSignal.timeout(5000) })
    .then(() => true).catch(() => false)
  if (!hasNetwork) {
    console.log('NOTE: instagram.com unreachable — tests 5–8 will be skipped (network firewall on this machine)\n')
  }
