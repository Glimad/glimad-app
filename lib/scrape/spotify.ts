// Spotify Web API — public artist/user data, client credentials flow
// Requires SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET env vars
// No user OAuth required for public artist profiles
...
export interface SpotifyNormalized {
  ...
  avg_views: number          // not applicable
  avg_likes: number          // not applicable
  avg_comments: number       // not applicable
  ...
  monthly_listeners: null    // not available in public API
  viral_spike: null
  // ── Spotify-specific ──────────────────────────────────────────────────────
  popularity: number
  genres: string[]
...
export async function scrapeSpotify(handle: string): Promise<{ raw: SpotifyRawData; normalized: SpotifyNormalized }> {
  ...
  // handle can be a Spotify artist URI (spotify:artist:ID), a URL, or a plain name/handle
  ...
  const artistData = await spotifyFetch(`/artists/${artistId}`, token)
  if (!artistData?.id) throw new Error(`Spotify artist data empty for id: ${artistId}`)

  // Fetch top tracks (market=US for public access)
  const tracksData = await spotifyFetch(`/artists/${artistId}/top-tracks?market=US`, token)
