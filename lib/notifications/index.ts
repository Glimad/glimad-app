...
const COPY = {
  en: {
    mission_reminder: {
      ...
      body: (template: string) =>
        `The mission <strong>${template}</strong> is waiting for your input for more than 24 hours.`,
  ...
  es: {
    mission_reminder: {
      ...
      body: (template: string) =>
        `La misión <strong>${template}</strong> lleva más de 24 horas esperando tu respuesta.`,
...
function getCopy(locale: string): (typeof COPY)["en"] {
  return (
    locale in COPY ? COPY[locale as SupportedLocale] : COPY.en
  ) as (typeof COPY)["en"];
}
...
export async function sendMissionReminders(admin: AdminClient) {
  ...
  for (const mission of staleMissions ?? []) {
    ...
    const userLocale: string =
      (authUser.user.user_metadata?.locale as string) ?? "en";
    const c = getCopy(userLocale).mission_reminder;

    const template = mission.template_code
      .replace(/_V\d+$/, "")
      .replace(/_/g, " ");
    ...
    await sendNotification(admin, {
      ...
      metadata: {
        ...
        template_code: mission.template_code,
      },
...
export async function sendWeeklyDigests(admin: AdminClient) {
  ...
  for (const project of projects ?? []) {
    ...
    // Aggregate stats
    const [missionsResult, contentResult, metricsNow, metricsWeekAgo] =
      await Promise.all([
        admin
          .from("mission_instances")
