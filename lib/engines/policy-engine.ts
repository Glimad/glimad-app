...
const PRIORITY_BASE: Record<number, number> = {
  0: 100,
  1: 80,
  2: 60,
  3: 40,
...
export async function runPolicyEngine(
  ...
  const inputHashRaw = createHash("sha256")
    .update(
      JSON.stringify({
        ...
        score: phaseResult.capabilityScore,
        inflexion: inflexion?.type ?? null,
        projectId,
      }),
    )
    .digest("hex");
  ...
  if (lastPolicyRun && !force) {
    const ageMinutes =
      (Date.now() - new Date(lastPolicyRun.created_at).getTime()) / 60000;
    if (ageMinutes < 5) {
  ...
  const hasBurnoutRisk = signals30d.some(
    (s) =>
      s.signal_key === "consistency_gap" ||
      s.signal_key === "downgrade_candidate",
  );
  ...
  const activeCodes = new Set(
    (activeInstances ?? []).map((i) => i.template_code),
  );
  ...
  const hasWinnerFormat =
    !!(facts["content.winner_format"] as
      | { confidence?: number }
      | null
      | undefined) &&
    ((facts["content.winner_format"] as { confidence?: number })?.confidence ??
      0) >= 0.8;
  ...
  const hasEngagementPlateau =
    inflexion?.type === "plateau" ||
    signals30d.some(
      (s) =>
        s.signal_key === "inflexion_detected" &&
  ...
  const { data: allTemplates } = await admin
    ...
    .select(
      "template_code, type, phase_min, phase_max, cooldown_hours, credit_cost_allowance, credit_cost_premium, priority_class",
    )
    .eq("active", true);

  const phaseRank: Record<PhaseCode, number> = {
    F0: 0,
    F1: 1,
    F2: 2,
    F3: 3,
  ...
  for (const template of allTemplates ?? []) {
    ...
    // Phase gate
    const minRank = template.phase_min
      ? phaseRank[template.phase_min as PhaseCode]
      : 0;
    const maxRank = template.phase_max
      ? phaseRank[template.phase_max as PhaseCode]
      : 7;
    ...
    if (inflexion && INFLEXION_MISSIONS[inflexion.type] === code)
      reasons.push(`inflexion:${inflexion.type}`);
    if (PHASE_MISSIONS[phaseResult.phase]?.includes(code))
      reasons.push(`phase:${phaseResult.phase}`);
    if (!completedMap.has(code)) reasons.push("new");
    if (hasBurnoutRisk && template.credit_cost_allowance > 10)
      reasons.push("downgrade_penalized");
