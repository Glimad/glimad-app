...
import { appendSignal, readAllFacts } from "@/lib/brain/index";
...
export async function approveContent(
  ...
  let platform = typeof content.platform === "string" ? content.platform : null;

  // If platform is null, fetch platforms.focus from brain_facts
  if (!platform) {
    const facts = await readAllFacts(admin, projectId);
    const focusPlatform = facts["platforms.focus"];
    if (
      focusPlatform &&
      typeof focusPlatform === "object" &&
      "platform" in focusPlatform
    ) {
      platform =
        typeof (focusPlatform as { platform: unknown }).platform === "string"
          ? (focusPlatform as { platform: string }).platform
          : null;
    } else if (typeof focusPlatform === "string") {
