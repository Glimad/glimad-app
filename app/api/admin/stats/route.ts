...
export async function GET(request: Request) {
  ...
  if (!project?.is_admin) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  ...
  const usersWithMissions = new Set(missionUsers?.map(m => m.project_id) ?? []).size
  ...
  const missionStats: Record<string, { total: number; completed: number; abandoned: number; totalMinutes: number }> = {}

  for (const inst of allMissionInstances ?? []) {
    if (!missionStats[inst.template_code]) {
      missionStats[inst.template_code] = { total: 0, completed: 0, abandoned: 0, totalMinutes: 0 }
    }
    missionStats[inst.template_code].total++
    if (inst.status === 'completed') {
      missionStats[inst.template_code].completed++
      if (inst.started_at && inst.completed_at) {
        const mins = (new Date(inst.completed_at).getTime() - new Date(inst.started_at).getTime()) / 60000
  ...
  const missionAnalytics = Object.entries(missionStats).map(([code, s]) => ({
    template_code: code,
    total: s.total,
    completed: s.completed,
    completion_rate: s.total > 0 ? Math.round((s.completed / s.total) * 100) : 0,
    abandoned: s.abandoned,
    avg_minutes: s.completed > 0 ? Math.round(s.totalMinutes / s.completed) : null,
  ...
  const reasonTotals: Record<string, { allowance: number; premium: number; count: number }> = {}
  for (const row of ledgerRows ?? []) {
    ...
    reasonTotals[row.reason_key].allowance += Math.abs(row.amount_allowance ?? 0)
...
export async function PATCH(request: Request) {
  ...
  await admin
    .from('feature_flags')
    .upsert({ flag_key, enabled, project_id: null, updated_at: new Date().toISOString() }, { onConflict: 'flag_key,project_id' })
