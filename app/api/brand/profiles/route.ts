/**
 * app/api/brand/profiles/route.ts
 * GET /api/brand/profiles - List opted-in creator profiles
 */
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const nicheFilter = url.searchParams.get("niche") || undefined;
    ...
    const limit = Math.min(
      ...
      keyValidation.key.maxResultsPerRequest,
    );
    const offset = parseInt(url.searchParams.get("offset") || "0");

    // Call handler
    const result = await BrandAPIHandlers.handleGetProfiles(
      admin,
      nicheFilter,
      tierFilter,
      minScore,
