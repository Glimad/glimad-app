...
export default async function NotificationsPage() {
  ...
  const messages = (await import(`@/messages/${locale}/notifications.json`)).default as Record<string, unknown>
    ...
    <div className="text-white max-w-2xl mx-auto px-4 pt-6 pb-12">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          {unread.length > 0 && (
            <p className="text-zinc-500 text-sm mt-0.5">{t('unread', { count: unread.length })}</p>
          )}
        </div>
        {unread.length > 0 && (
      ...
      {items.length === 0 ? (
        <div className="bg-zinc-900 rounded-xl p-10 border border-zinc-800 text-center">
          ...
          <p className="text-zinc-500 text-sm mt-1">{t('no_notifications_sub')}</p>
        </div>
      ) : (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <p className="text-sm font-semibold text-white">{getNotifTitle(n)}</p>
                    {!n.read_at && (
                      <span className="w-2 h-2 bg-violet-500 rounded-full flex-shrink-0" />
                    )}
                  </div>
                  <p className="text-sm text-zinc-400 leading-snug">{getNotifBody(n)}</p>
