...
export default function MissionMap({ missions, t }: Props) {
  ...
  async function startMission(templateCode: string) {
    ...
    try {
      ...
      if (!data.instance_id || data.instance_id === "undefined") {
        setError("Could not start mission. Please try again.");
        return;
      }
      ...
      <div className="space-y-3">
        ...
        {missions.map((mission) => (
          <div key={mission.template_code} className="relative flex gap-4">
            {/* Node dot */}
            <div
              className={`relative z-10 flex-shrink-0 w-12 h-12 rounded-full border-2 flex items-center justify-center text-sm font-bold
              ${
                mission.status === "completed"
                  ? "border-green-700 bg-green-950 text-green-400"
                  : mission.status === "active"
                    ? "border-violet-500 bg-violet-950 text-violet-300 animate-pulse"
            ...
            <div
              ...
              onClick={() => {
                if (mission.status === "available")
                  setSelected(
                    selected === mission.template_code
                      ? null
                      : mission.template_code,
                  );
                if (mission.status === "active" && mission.instance_id)
                  resumeMission(mission.instance_id);
              }}
            >
              ...
              {selected === mission.template_code &&
                mission.status === "available" && (
                  <div className="mt-3 pt-3 border-t border-zinc-700">
                    ...
                    {(mission.credit_cost_allowance > 0 ||
                      mission.credit_cost_premium > 0) && (
                      <p className="text-xs text-zinc-500 mb-2">
                        {t.credits}:{" "}
                        {mission.credit_cost_allowance > 0
                          ? `${mission.credit_cost_allowance} ${t.credit_allowance}`
                          : ""}
                        {mission.credit_cost_premium > 0
                          ? `${mission.credit_cost_premium} ${t.credit_premium}`
                          : ""}
                      </p>
                    )}
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        startMission(mission.template_code);
                      }}
